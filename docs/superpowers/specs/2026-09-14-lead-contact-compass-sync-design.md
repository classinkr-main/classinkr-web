...
- 사용자 결정 (모두 2026-09-14)
  - 자동 반영
  - 이탈 리드는 종료로 반영
  - Vercel Pro 전환 뒤에는 전용 크론으로 옮기고, 근무 시간 기준 하루 최대 5번 돌린다
...
- 평일 근무 시간에 하루 5번 자동으로 반영하고, 한 방향으로만 바꾼다.
...
- 작성자(`actor`)가 자동 기록 작성자(`Claude`·`BD시트`·`시트`·`시트 동기화`·`시스템`·`system`)가 아니다.
  - 근거(코드 리뷰, 2026-09-14): Compass 설명회 명단 동기화가 신규 리드를 만들며 `note`/`BD시트` 메모를 자동으로 넣는다. 시트 크론은 종료를 전파하며 `시트 동기화` 메모를 남기고, 시트 백필·중복 병합 스크립트도 `시트`·`Claude` 메모를 남긴다.
...
| 실행 자리 | `app/api/cron/lead-contact-sync/route.ts` (신규 전용 크론) | `vercel.json` 한 줄로 **평일 KST 09:50·10:50·13:50·15:50·17:50** 에 돈다(`50 0,1,4,6,8 * * 1-5`, UTC). 예산 45초, maxDuration 60초 |
...
**실행 자리 세부**
- **처음 설계와 바뀐 점**: 처음에는 매시간 도는 dispatch 슬롯(아침 카드와 같은 경로)에 얹었다. Vercel 팀이 Pro 로 바뀌어 하루 여러 번 도는 크론을 분 단위로 정확히 둘 수 있게 됐고, 사용자 결정에 따라 전용 크론으로 분리했다. dispatch 라우트는 건드리지 않는다.
- **시각**
  - 운영 아침 카드는 평일 11:00 KST 발송이다(`site_settings.notification_schedule_json`). 10:50 실행 덕분에 카드 숫자가 최신 상태로 나간다.
  - 09:50 은 하루 시작, 13:50·15:50 은 오후, 17:50 은 퇴근 전이다.
  - UTC 0~8시는 KST 9~17시와 같은 날짜라, 요일 필터 `1-5` 가 KST 평일과 그대로 맞는다.
- **확인**: `?dryRun=true` 는 판정 건수만 돌려주고 쓰지 않는다.
- **응답 코드**: 크론 대시보드에서 문제가 보이도록 나눈다 — 정상 200, `bridge_down` 503, `failed` 500, `timeout` 504. 예상 못 한 예외는 500 에 일반 문구만 싣는다.
- **빌드 선행 조건**: `scripts/check-vercel-crons.mjs`(prebuild)가 표현식당 하루 1회 초과를 막는다. 크론 정리 작업(별도 세션)이 Pro 기준으로 가드와 AGENTS.md 크론 규칙을 바꾼 뒤에 병합한다.
- **계약 테스트**: `tests/api/cron-lead-contact-sync-route.test.ts` 가 vercel.json 을 읽어 사용자 결정을 고정한다. 경로당 1개, UTC 0~8시, 평일, 하루 5회 이하인지 본다.
...
- **매칭·지연**: 전화 키로만 매칭한다. 근무 시간에는 최대 약 3시간 늦게 반영된다(10:50→13:50). 금요일 17:50 이후 처리분은 월요일 09:50 에 반영된다. 주말에는 돌지 않는다.
...
- **전용 크론 라우트** (`tests/api/cron-lead-contact-sync-route.test.ts`)
  - Bearer 인증: 없거나 틀리면 401 이고 반영하지 않는다
  - 정상 200, `dryRun=true` 는 쓰지 않는다
  - 상태별 응답 코드(503·500·504)와 예외 시 일반 문구
  - vercel.json 일정 계약(평일 근무 시간, 하루 5회 이하)
...
1. **첫 예약 실행 전에** `GET /api/cron/lead-contact-sync?dryRun=true` 를 `Authorization: Bearer ${CRON_SECRET}` 로 호출한다. `toContacted`·`toClosed` 가 기준값과 비슷한 규모인지 본다. 유입이 늘어난 만큼은 차이가 난다.
2. 다음 예약 시각이 지나면 두 가지를 확인한다.
   - Vercel 크론 대시보드에 200 이 찍혔는지
   - 읽기 전용 프로브로 잰 리드 상태 분포

...
- [active/playbook/README.md](./active/playbook/README.md)
  - **파트별 운영 플레이북.** 작업이 어느 파트(홈/어드민/콘텐츠/그로스/챗봇/플랫폼)인지 판별 → 담당 에이전트(`.claude/agents/`)·가이드·공통 철칙 적용. 업무 분담의 진입점. 공통 함정·체크리스트는 [active/playbook/work-flow-patterns.md](./active/playbook/work-flow-patterns.md).

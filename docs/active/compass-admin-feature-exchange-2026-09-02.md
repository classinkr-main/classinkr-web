...
§5.1 순서 중 Admin 쪽은 하위 티어 에이전트 7개(파일 소유 분리, 병렬)가 구현하고 오케스트레이터가 diff 검토·게이트 통과 후 과제 단위로 커밋했다. 각 항목은 타입체크·ESLint·해당 vitest 를 통과했다.

| 순서 | 항목 | 상태 | 비고 |
|---|---|---|---|
| 2 | 재유입 병합 + 응대 판정 경과 기준 | 완료 | `phone_key` 생성 컬럼 마이그레이션(20260914) 적용 전에는 원문·숫자만 비교로 폴백. 행사 신청은 다른 행사 토큰이 있으면 병합하지 않는다. `isReinflowAwaitingContact` 배선은 후속 |
| 3 | timing-safe 크론 시크릿 헬퍼 | 완료(Admin 11곳) | 응답 코드·메시지 불변. Compass 7곳은 후속 |
| 4 | adset 브리지 뷰 소비, 소재 원본 이미지 확대 | 완료 | 소비처 0건이던 `compass_adsets_v` 첫 소비 |
| 6 | 자동 새로고침, 네비게이션 진행바, 되돌리기 | 완료 | 자동 새로고침은 force 없이 TTL 을 존중한다(탭당 매분 서버 재계산 방지). 되돌리기는 상태 변경 단건·일괄 |
| 11 | 한 줄 태스크 파싱, 전환 계정 수 최초 결제월 1회 | 완료(방문 버킷 제외) | 파싱된 담당자는 정본 명부로 ownerKey 를 되짚어 실제 배정까지 이어진다. 전환 계정 수는 현재 회계연도 원천 한계로 "조회 기간 내 최초 결제 기준" 표기 |
| 1 | Compass 웹훅 정규화 단일화, `lower(email)` 인덱스, 폼 답변 보존 | PR 열림 | Compass 저장소 별도 브랜치. 신규 리드의 커스텀 답변은 `inflow` 가 아니라 `note` 활동으로 남긴다(`inflow` 존재 자체를 재유입으로 세는 화면이 있어서) |
| 5·7·8·9·10 | 알림 크론, Compass UX 이식, 원장 헤더 열 해석, 우선순위 점수, 역방향 명부 뷰 | 미착수 | 다음 배치 |

운영 반영 전 확인: §5.2 의 세 가지(Meta 웹훅 구독 수, 캘린더 소유 계정, Compass Vercel 플랜)에 더해 `20260914_leads_phone_key.sql` 적용 후 `npm run check:db` 로 프로브가 초록인지 본다.

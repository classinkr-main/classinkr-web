# 하드웨어 SCM 탭 — 정체성 · UI · 기능 (reference)

기준 시점: 2026-06-29
대상: `/admin/hardware` → [components/admin/hardware/HardwareInventoryClient.tsx](../../components/admin/hardware/HardwareInventoryClient.tsx)
관련: [UX 계획·작업내역](./hardware-admin-ux-plan.md) · [hardware-ops 개념 문서](../hardware-ops/README.md) · [시트 해석](../hardware-ops/sheet-interpretation.md) · [DESIGN.md](../../DESIGN.md)

이 문서는 하드웨어 재고 화면을 **공급망(SCM) 운영 콘솔**로 보고, 그 정체성·UI·기능을 한곳에 고정한다. "지금 무엇이고, 어떻게 생겼고, 무엇을 하는가"의 단일 기준.

---

## 1. 정체성 (Identity)

- **한 줄 정의**: 클래스인 하드웨어(전자칠판/IFP·카메라·스탠드·부자재)의 **재고·입출고·예상 출고를 한 화면에서 추적하고 기록하는 운영 콘솔**.
- **주인공 엔티티**: 품목(`item`) · **물류 번호(lot)** · 입출고 기록(`movement`) · 예상 출고(`planned`).
- **진실의 원천**: `hardware_movements` **원장**. 재고는 따로 저장하지 않고 **원장에서 파생 계산**한다. 기록은 수정/취소(void)로 보정하되 원장 자체가 기준.
- **두 데이터 출처**:
  - `admin_manual` — 어드민에서 수기 입력(이 화면의 주 입력 경로)
  - `sheet_import` — Google 시트(`branch_hw_*` 미러) → 가져오기 RPC가 시트 이관분을 교체
- **무엇이 아닌가**: 단순 수량 입력표나 파트너 목록이 아니라, **물량 진실표 + 운영 워크플로**(입고→예약→확정→출고/취소).
- **사용 맥락**: 본사 운영자가 데스크톱/모바일(창고·현장)에서 빠르게 기록. 모바일 입력을 1급으로 본다.

---

## 2. UI 구조

### 2-1. 하위탭 (헤더 탭 스트립, 3개)

| 탭 | 정체성 | 구성 |
| --- | --- | --- |
| **홈** | 현황 + 예상 출고 워크플로 | **요약 밴드 5칸**(창고·가용·예정 대기·부족/주문 검토·시트 이관 신선도) · 이관 경고 · **예상 출고 큐**(등록·부분/전체/**선택 일괄** 확정) · 품목 카드·현재 재고(lot 칩) · 판매 요약 · 검색 · **사무실·샘플 재고(유닛 기준, 3-7)** · 샘플 트래커 · 알림 · 스냅샷 복원 |
| **입출고** | 기록 진입 | 입고 · 물량번호(lot별 입고 목록) / 출고 · 기간 집계. "빠른 기록"은 입고 보기에서는 **입고표**(2-3), 출고 보기에서는 빠른 기록 시트를 연다 |
| **내역** | 전체 원장 | 유형·제품 칩 필터 · lot/금액 표시 · 행별 수정·취소 |

### 2-2. 빠른 기록 시트 (슬라이드오버) — 핵심 입력 컴포넌트

- 우측 드로어(모바일 풀폭). **진입점 통일**: 재고 행·예상 큐·제품 칩·런처·우하단 FAB가 모두 같은 시트를 그 자리에서 prefill로 연다(탭 전환 없음).
- 접근성: `role="dialog" aria-modal`, Escape 닫기, body 스크롤 잠금, autofocus·포커스 트랩·복원, 백드롭 click.
- 구성: 액션 세그먼트 → 제품(칩/검색) → 수량 → 조건부(출고=도착·lot / 입고=lot·시트 상세) → `상세` 접기 → 미리보기 → 저장. 검증 에러는 시트 내부에 노출.
- 모드: 신규 기록 / **수정(editingId)** — 같은 시트가 헤더·버튼 라벨만 바꿔 재사용.

### 2-2a. 한 화면 입고표 (`InboundSheet`, 2026-09-15 — 운영자가 시안 A 선택)

새 입고는 전부 이 시트로 연다. 입출고 탭 "빠른 기록"(입고 보기), 재고 표·알림·검색의 "입고" 퀵버튼, 빠른 기록 시트의
"입고" 탭이 모두 여기로 온다. 기존 입고 기록 **수정**만 빠른 기록 시트(단건)에 남는다.

- 위에서 한 번 고르는 값
  - **물량번호**: 새 물량 추천(날짜가 가장 최근인 `영문+숫자` lot의 번호를 올린다. C2→C3, H8→H9, `H7(과사람)`·`FY24-25`·`Sample`은 제외), 최근 lot 칩(날짜·대수), 직접 입력
  - **입고일**: 오늘·어제(로컬 날짜), **수입자** 칩, **기본 보관처** 칩(오산 창고·인천 더조은·클래스인 사무실·고객사 직송)
- 품목 표: 품목 | 수량 | 단가 USD(최근 입고 단가로 미리 채움) | 금액(자동) | 보관처(행마다 바꿀 수 있음). 주요 품목 5종이 먼저 나오고 나머지는 "품목 추가"로 넣는다.
- 보조 기능
  - "{직전 lot} 구성 불러오기"
  - "엑셀에서 붙여넣기": 품목 이름은 **정확 일치**만 인정한다(T1이 DT1·T1(promoted)에 걸리지 않음)
  - Enter로 다음 행 이동, 한글 조합 중 Enter 무시, Cmd/Ctrl+Enter로 저장
- 저장
  - `POST /api/admin/hardware/movements` 한 번으로 보낸다. 일부만 실패하면 실패한 행만 시트에 남긴다.
  - 보관처 라벨은 `toLocation`(재고 위치, 서버가 정규화)과 `storageLocation`(원문) 양쪽에 싣는다.
  - 사무실 보관 행이 있으면 "사무실 입고분 N대를 샘플 유닛으로 등록"(기본 켬)이 행마다 office 상태 유닛을 발급한다.
- 시트 이관이 신선도 기준(홈 스트립과 같은 판정)을 넘기면, 새 물량 추천 아래에 "그 뒤에 들어온 물량번호가 빠져 있을 수 있다"는 안내를 붙인다. 원장에 없는 C2를 C2로 추천하던 사례(2026-09-15)가 있었다.
- 모델은 순수 함수 `inbound-sheet-model.ts`에 있다(테스트 `tests/admin/hardware-inbound-sheet-model.test.ts`).

### 2-3. 보조 오버레이

- **CRM 오더 확인 모달**: 실제 출고 저장 시 CRM 실제 오더와 연동할지 확인(배송 예정 등록 시에는 뜨지 않음).
- **기록 취소 확인 모달**: 사유 입력 + 파괴적 액션 스타일.

### 2-4. 디자인 언어

- [DESIGN.md](../../DESIGN.md) 팔레트 + **운영 상태 스케일**(green=주요 액션 단일, danger=출고·부족·취소, warning=예정·검토, success=입고·확정).
- 11px 미만 텍스트 없음, 경량 그림자, hairline 보더, 행 내 빠른 액션은 중립(hover에서 의도색).
- 재사용 부품: `AdminTabs` · `StatCard` · `viz/SectionCard·Panel·EmptyState`.

---

## 3. 기능 (Functions)

### 3-1. 입출고 프리셋 (`ENTRY_PRESETS`, shared.tsx — 2026-09-15 코드 기준)

| 프리셋 | 유형 | from → to | 비고 |
| --- | --- | --- | --- |
| 판매 출고 | outbound | 창고 → 고객 | 저장 시 CRM 오더 확인 |
| 배송 예정 | outbound(예정) | 창고 → 고객 | 가용에서 미리 차감(예약) |
| 샘플 대여 | outbound | 사무실(기본)·창고 → 샘플 | 고객사 필수, 사무실 가용 유닛 칩에서 고름 |
| 입고 | inbound | → 보관처 | **한 화면 입고표**로 연다(2-2a) |
| 샘플 반환 | return | 샘플 → 사무실 | 상세 모드 |
| 샘플 배정 | transfer | 창고 → 사무실 | 상세 모드, 수량만큼 사무실 유닛 발급 |
| 고객 반납 | return | 고객 → 창고 | 상세 모드 |
| 수리 | repair | 창고 → 수리 | 상세 모드 |
| 실사 조정 | adjust | ± 창고 | 상세 모드 |

### 3-2. 입고

한 화면 입고표(2-2a)가 품목·수량·입고일·물류No(lot)·단가(USD)·금액(USD)·보관처·수입자·시리얼을 한 번에 받는다.
금액(CNY)·메모·상태 같은 드문 필드는 기존 기록 수정(빠른 기록 시트 단건)에서 고친다.

### 3-3. 예상 출고 워크플로 (핵심)

- **등록**: 배송 예정 movement 생성 → 가용에서 차감, 홈 큐에 표시.
- **확정**: 예정 → 실제 출고로 **전환(convert)**. 출고일 선택. **부분 수량 확정** 지원 — 출고 N대 생성 + 잔여(전체−N)는 예정 유지.
- **선택 일괄 확정(2026-09-14)**: 행·딜 체크박스(Shift 범위 선택), 빠른 선택("전체 N건"·"30일+ 미확정"·해제),
  하단 작업 바의 공통 확정일로 **순차** 확정한다. 순차인 이유 — 각 확정이 로트 잔량을 바꾸므로 다음 건의
  배정이 앞 건을 반영해야 하고, 건별 권한 검사(`hardware.finalize`)·감사 로그가 그대로 남는다. 실패 건은
  사유를 행에 남기고 선택에 남겨 재시도한다.
- **로트가 모자라도 확정을 막지 않는다(운영자 결정 2026-09-14).** 해석 가능한 만큼 FIFO 로 배정하고
  나머지는 로트 미지정(`lot_no` NULL) 한 줄로 기록한다. 로트 기록이 전혀 없는 품목(OPS·케이블 등)도 출고를
  남길 수 있다. 운영자가 로트를 **직접 지정**한 경우만 그 로트 잔량을 검사한다.

### 3-4. lot(물류 번호) 추적

- lot 키 = 수기 `lot_no`, 시트행은 `reference_no`(물류No) 폴백. 운영 원장(시트 임포트)은 `lot_no` 가 비어
  있고 로트는 `reference_no` 에만 있다(2026-09-14 실측 385건 전부).
- **lot 잔량 정본 = `resolveHardwareLotBalances`**(`lib/repositories/hardware-inventory.ts`). 화면 표시와
  새 출고 자동 배정·예정 확정 배정이 **모두 이 함수 하나**를 쓴다. 규칙(순서 무관·결정적):
  1. 로트 키가 있는 이동을 로트별로 합산한다 — 입고/반납(+) · 출고(예정 포함, −) · 보정(방향대로) · 이동/수리(0).
  2. **음수가 된 로트의 초과분은 FIFO(가장 오래된 양수 로트부터)로 흡수**한다. 설치 기록에 현행 세대 이름을
     붙여 로트를 넘겨 출고한 물량(예: STD1 H8 입고 19 · 출고 28)은 실제로 옛 로트에서 나간 것이다.
  3. **로트 없는 감소분(출고·음수 보정)도 FIFO 로 차감**한다. 로트 없는 증가분은 귀속시키지 않는다.
- FIFO 순서: `FY` < `H숫자` < 그 밖(`C1`·`Sample` 등, 처음 본 날짜순).
- 예전 규칙(단순 합산 + 음수 숨김)은 실물에 없는 옛 로트를 재고로 보여줬다(STD1 H4·H5·H6, T1 H6 —
  운영자 확인: 실제는 H8·C1 뿐). 날짜순 재생을 쓰지 않는 이유는 시트 원장에 출고일이 입고일보다 앞서는
  행이 흔하기 때문이다.

### 3-5. 기록 수정·취소

- **수정(update)**: `admin_manual`·미전환·미취소 기록만. 내역·예상 큐 `수정`이 신규 생성이 아니라 같은 행을 갱신.
- **취소(void)**: 사유와 함께 무효화. 시트 출처 기록은 직접 취소 불가(가져오기가 관리).

### 3-6. 시트 가져오기 / 재고 계산 / 알림

- **가져오기**: Google 시트 강제 싱크 → 스냅샷 백업 → `hardware_movements`의 시트 이관분 교체.
  자동이 아니다 — 크론은 시트 → `branch_hw_*` 미러까지만 매일 돌고, 원장 반영은 이 버튼을 눌러야 일어난다.
- **재고 파생**: 원장에서 위치별 잔량·예상 가용(예정 차감)·30일 출고·주문 시점(리드타임 반영)을 계산.
- **알림**: 부족·주문 검토·배송 예정.

**시트 → 원장 규칙 (2026-09-14 운영 시트 실측으로 확정)** — `buildHardwareSheetImportRows`(순수 함수)·`parseStock`:

1. **재고현황 입고·출고 블록은 로트 열 합이 정본이다.** "합계" 셀은 시트 수식이라, 새 로트 열을 붙일 때 범위를
   늘리지 않으면 조용히 틀린다. 실측: 86" IFP 입고 합계 344인데 로트 열 합은 359(C2 15대 누락) → 현재고가 30이 아니라
   15로 싱크됐다. 두 값이 다르면 로트 열 합을 쓰고 `raw.sheet_total_mismatch`에 시트 합계를 남긴다. 재고 현황
   블록(OPS "사무실 보유" 등)은 로트 열에 글자가 있어 합계 셀을 그대로 쓴다.
2. **재고현황 출고는 진행 상태를 가리지 않고 로트 열별로 합산한다.** 즉 로트가 적힌 "배송 예정" 행도 이미 빠진
   수치다. 실측: 75" IFP H8 출고 10 = 설치 완료 5 + 배송 예정 5, 같은 형태 6개 로트·16대. 원장은 예정 출고를
   창고에서 빼지 않고 가용에서 따로 빼므로, **보정 목표 = 시트 현재고 + 시트가 이미 뺀 로트 지정 예정 수량**이다.
   로트 없는 예정(날짜·물류No 없는 수주 대기, 실측 67행·163대)은 시트 재고현황에 안 잡히므로 더하지 않는다.
3. **30일 출고 추세는 실제 출고만, 날짜가 있는 시트 행만 센다.** 예정은 가용에서 이미 빠지고, 날짜 없는 시트
   행은 `created_at`(임포트 시각) 폴백 때문에 가져올 때마다 "최근 출고"가 됐다(드라이런 86" IFP 9 → 93).
4. 입고·출고 탭은 행 끝을 열어 읽는다(`A1:Z`). 예전 고정 500행이면 약 한 달 뒤(출고 409행, 37일에 94행 증가)
   가장 최근 행부터 에러 없이 잘릴 참이었다.

**위치 정규화 (`normalizeLocationName`, 2026-09-15)** — 쓸 때와 읽을 때(잔량 키) 모두 적용되므로 규칙을 바꾸면 기존 행도
즉시 새 위치로 읽힌다. 대시보드 캐시 키는 `hardware-dashboard-v3`로 올렸다.

1. 샘플·대여·데모·demo·sample → **샘플** (가장 먼저 판정)
2. 사무실·office·**클래스인·ClassIn** → **사무실**. 시트 보관처 "클래스인"(본사 사무실·쇼룸 보관분, 원장 28대)이 예전에는 이름 없는 별도 위치로 남아 어디에도 안 보였다.
3. 수리 → **수리 표기에만** 맞춘다: repair, A/S, 독립 단어 AS(·AS 센터), `수리`(뒤에 학원·논술·영역·수학·과학·탐구·교육·아카데미가 오면 제외). 예전 `/수리|a\/?s/`는 "ClassIn"·"Master"의 as와 "대치수리학원"을 수리 위치로 보냈다.
4. 창고·warehouse·인천 더조은·FPL → 창고, 고객·고객사 → 고객, 나머지는 원문 그대로(고객사 이름)
5. 시트 수입자는 회사명이라 위치 정규화를 거치지 않는다(`from_location` 원문).

**미해결 — 어드민 확정과 교체(replace) 가져오기의 이중 계상.** `replace_hardware_sheet_import`는 시트 이관 행을
통째로 지우고 다시 넣는다. 전환된 예정 행도 지워지고, 어드민 확정으로 생긴 `admin_manual` 출고는 그대로 남는다.
그래서 시트 행을 어드민에서 확정한 뒤 가져오면, 시트가 아직 "배송 예정"이든 "설치 완료"로 바뀌었든 같은 물량이
두 번 잡힌다. 시트에는 안정적인 행 ID가 없어 행 단위 대조로는 풀 수 없다. 권장안은 "가져오기 시 시트가 이긴다"이다:
교체 트랜잭션 안에서 시트 행에서 전환된 `admin_manual` 확정을 void 하고, 결과에 "시트 미반영 확정 N건"을 표시한다.
적용 전까지는 **가져오기를 먼저 돌리고, 그 뒤 시트에 아직 예정으로 남은 행만 확정**하는 순서로 운영한다.
2026-09-14 기준 운영 원장의 `admin_manual`은 0건이라 지금 가져오기에는 영향이 없다.

### 3-7. 사무실·샘플 재고 풀 — 유닛(관리번호) 기준 (2026-09-15 운영자 결정)

- **정본은 유닛이다.**
  - 사무실 보관(`office`)이 **가용**, 전시·사내 사용(`showroom`, 쇼룸·KC인증 등)은 사무실 보유지만 가용이 아니다.
  - 대여(`loaned`)가 나간 샘플이다.
  - 시트는 샘플이 사무실에서 나갔는지 기록하지 않으므로, 원장의 사무실·샘플 잔량은 **교차 확인용**이다. 같은 FY24-25 사무실 샘플이 입고(보관처 클래스인) 16대와 출고(목적지 클래스인, Sample) 15대로 두 번 적힌 사례가 있다.
- 홈 섹션 `OfficeSamplePoolSection`(예전 "재고 위치 맵" 자리)
  - 타일 3개: 판매 재고·창고 / 클래스인 사무실 / 나간 샘플(90일+·회수 예정 14일)
  - 품목 표: 창고·가용 | 사무실·가용·전시 | 샘플·90일+
  - 행을 펼치면 유닛 목록과 일괄 액션이 나온다: 전시로 / 사무실 보관으로 / 상태 정정(메모 필수).
  - [대여]·[반납]은 빠른 기록의 샘플 프리셋으로 연다.
  - 원장과 차이가 있으면 "원장과 N대 차이" 배지를 붙인다.
  - 집계는 순수 모델 `office-sample-pool.ts`가 한다. 판촉형은 합계에서 빼고 별도로 표시한다.
- 상태 전이(`SAMPLE_EVENT_TRANSITIONS`, lib/repositories/hardware-samples.ts)
  - loan: office·repair → loaned. 전시 중에는 먼저 보관으로 옮겨야 한다.
  - return: loaned → office
  - showcase: office → showroom
  - store: showroom·repair → office
  - repair·convert·retire는 showroom에서도 허용한다.
  - adjust에 `nextStatus`를 주면 상태를 정정하고, 이때 사유 메모가 필수다.
  - 사무실로 들어오는 전이는 고객·대여일·회수 예정일을 비운다.
  - 샘플 API는 이제 규칙 오류를 400/404/409로 응답하고 감사 로그(`hardware.sample.*`)를 남긴다.
- 샘플 트래커의 원장 백필 버튼은 **확인창을 거친다**("원장 차이 N대 확인" → 품목별 수량 목록 → 모두 등록).
  - 사무실 정규화 뒤 원장 사무실 잔량이 28대 늘어 이 차이가 32대가 됐다. 한 번 누르면 실물 없는 유닛이 생길 수 있었다.
  - 정합 대조의 사무실 유닛 수에는 전시 유닛도 센다.
- 운영 데이터 상태(2026-09-15): 유닛 33대 **전부 loaned**. 고객 미상 16, 클래스인 쇼룸 2, 클래스인 (KC인증) 1. 새 섹션의 상태 정정으로 정리한다.
  - 전시(`showroom`)로 옮기는 것은 마이그레이션 `20260915_hardware_sample_showroom_status.sql` 적용 뒤에 가능하다.
  - 적용 전에는 "전시 상태는 DB 업데이트 적용 후 사용할 수 있습니다."로 막힌다.

---

## 4. 데이터 모델 / API (요약)

테이블: `hardware_items` · `hardware_movements` · `hardware_import_runs` · `hardware_sheet_import_snapshots` · `branch_hw_inbound|outbound|stock`(시트 미러).

`hardware_movements` 주요 컬럼: `item_id, product_name, movement_type(inbound|outbound|return|transfer|repair|adjust), quantity, occurred_at, from_location, to_location, owner, status, reference_no, memo, serials[], lot_no, unit_price, amount_usd, amount_cny, storage_location, importer, source(admin_manual|sheet_import), import_run_id, raw, voided_*, converted_from/to_movement_id`.

RPC: `confirm_hardware_planned_movement_v3(uuid,text,date,int,jsonb)`(**앱이 계산한 로트 배정을 받아 기록**, 로트 미지정 허용) → `confirm_hardware_planned_movement_v2`(SQL 내부 FIFO, `lot_no` 만 봄) → 레거시 `confirm_hardware_planned_movement` 순으로 앱이 폴백 · `replace_hardware_sheet_import` · `restore_hardware_sheet_import_snapshot`.

API:
- `GET /api/admin/hardware` — 대시보드(items/stock/movements/recentOutbound/plannedMovements/alerts/totals/importRun)
- `POST /api/admin/hardware/movements` — 기록 생성
- `PATCH /api/admin/hardware/movements/[id]` — `{action: confirm-planned | void | update}`(confirm은 `confirmQty` 옵션)
- `POST /api/admin/hardware/import-sheet` — 싱크·백업 후 이관
- `GET /api/admin/hardware/crm-orders` — 출고 시 CRM 후보

---

## 5. movement 생애주기

```
생성(admin_manual | sheet_import)
  → [배송 예정] ──확정(convert)──▶ 실제 출고
        │                         · 전체: 예정 void + converted_to
        │                         · 부분: 예정 quantity 감소(잔여 유지)
        └─ void(취소) / update(수정, admin_manual·미전환만)
```

---

## 6. 마이그레이션 (라이브 적용 완료)

`20260626_*_ledger` · `20260626_*_workflow_guards` · `20260627_*_snapshots` · `20260628_*_lot` · `20260628_*_costing` · `20260629_*_partial_confirm` · `20260630_*_sheet_import_merge`(추가형 머지 RPC·라이브 적용, 플래그 off). 적용은 Supabase Management API query 엔드포인트로 수행(운영 메모 별도).

**적용 완료 (2026-09-14)**: `20260914_hardware_confirm_planned_v3.sql` — v3 확정 함수 **추가만**(v2·레거시 유지, 데이터 변경 없음).
운영 프로젝트 `pxbrsbovoobowpfarxmn`(서울, `docs/active/supabase-korea-migration-status.md`)에 적용하고 PostgREST
스키마를 다시 읽혔다. 검증: 권한은 service_role 만, 앱 경로(PostgREST)가 v3 에 도달해 존재하지 않는 예정 번호를
표준 문구("배송 예정 기록을 찾을 수 없습니다.")로 거절, 호출 전후 원장 385건·admin_manual 0건으로 변화 없음.
적용 시점 대기 중인 예정 출고는 33건 59대(전부 `lot_no` NULL)였다 — 이제 홈에서 단건·딜·선택 일괄로 확정할 수 있다.

**미적용 확인 (2026-09-14, 운영 함수 본문 실측)** — 파일은 있지만 운영 함수가 옛 본문이다.

- `20260630_hardware_sheet_import_inbound_costing.sql`: `replace_hardware_sheet_import`가 `unit_price`·`amount_usd`·
  `storage_location`을 넣지 않는다. 원장 385건 전부 금액 컬럼이 NULL이고, 화면은 `recoverMoneyFromRaw`로 raw에서
  복구해 보여준다.
- `20260630_hardware_sheet_import_merge_costing.sql`: `merge_hardware_sheet_import`도 금액을 넣지 않는다(플래그 off라 현재 경로 아님).
- `20260701_hardware_restore_snapshot_guard.sql`: `restore_hardware_sheet_import_snapshot`에 빈 스냅샷 거부 가드가 없다.
  같은 파일의 Item 2는 `crm_neo_customer_snapshots` 권한 정리라, 적용 전에 테이블 존재를 확인해야 한다.

적용 순서는 inbound_costing → merge_costing → restore_guard 이다. restore_guard는 inbound_costing의 restore 본문에 가드를 더한 것이다.

**미적용 (2026-09-15 작성)**: `20260915_hardware_sample_showroom_status.sql`
- 샘플 유닛 status에 `showroom`, 이벤트에 `showcase`·`store`를 더한다. 운영 제약 이름(`hardware_sample_units_status_check`·`hardware_sample_events_event_type_check`)을 확인했고, 데이터 변경은 없다.
- 적용 스크립트: `tmp/apply-hardware-sample-showroom-20260915.mjs`(gitignore, 전후 제약·유닛 분포 출력). 롤백 절차는 파일 머리 주석에 있다.

---

## 7. 추가형(누적) 시트 인제스트 — 설계 · cutover 런북

목표: 시트를 매번 통째 교체(delete+insert)하지 않고 동기화가 **누적**되도록. 대시보드는 이미 DB(`hardware_movements`) 기준이라 변경 대상은 인제스트뿐. (멀티에이전트 워크플로 + 적대적 검증으로 설계 확정 — 검증 4종 모두 초기안을 breaks 처리.)

핵심 설계:
- 시트엔 안정적 행 키가 없음(물류No 공란/임시·lot 다행, 비정규 날짜) → 콘텐츠 키 UPSERT는 **서로 다른 출고를 묵음 병합(과소집계)** 위험.
- 해법: **fingerprint(정체성)+digest(콘텐츠)**. `source_key = hash(제품·날짜·물류No·그룹 내 안정 ordinal)` — 같은 그룹은 ordinal로 항상 유일(구성상 충돌 불가). `source_digest`=콘텐츠 해시(편집 감지). 재고현황 보정은 제품당 1행(product-only 키).
- `merge_hardware_sheet_import` 4-pass: 신규 INSERT / 시트-tombstone 부활 / 변경분 in-place UPDATE / 사라진 행 soft-tombstone. **부재 >40% 시 fail-closed** 중단. converted·human_locked·타-사유 void 행은 불가침. admin_manual 무관.
- 부분 확정 잔여행은 `confirm`이 `human_locked_at` 스탬프 → 머지가 덮어쓰지 않음.
- 플래그 `HARDWARE_SHEET_ADDITIVE_MERGE`(기본 off). 롤백 = off (구 `replace`/`restore` RPC 그대로).

cutover 런북(코드 배포 후, 순서 중요):
1. 코드 배포(플래그 off). 마이그레이션 20260630은 이미 라이브.
2. **백필**: 가져오기 1회(아직 replace 경로) → 기존 행이 fingerprint `source_key`로 재기록. 충돌 시 unique index가 loud-fail(구성상 충돌 없음, 안전망).
3. `HARDWARE_SHEET_ADDITIVE_MERGE=1`.
4. 가져오기 재실행 → merge 경로. 카운트 `{inserted,updated,tombstoned,revived}` 확인(첫 머지는 digest 백필로 updated 큼=정상).
5. 패리티 확인: 직전 replace 결과와 첫 merge 결과의 `warehouseStock/availableStock/plannedOut/lotBalances` 동일.
- 문제 시 즉시 플래그 off → 다음 가져오기가 replace로 자가 치유.

최적화(누적이 천장을 제거 → fast-follow): `listAllHardwareMovements` 투영/기간 플로어 또는 `HARDWARE_INVENTORY_CACHE_TAG` 캐시 연결, 스냅샷 retention RPC.

## 8. 보류 / 다음

- **원장 가져오기 재실행**: 마지막 성공은 2026-08-08, 시트 미러는 매일 최신이다. 3-6 규칙 수정을 배포한 뒤
  "싱크 백업 후 가져오기"를 누른다. 드라이런(쓰기 0) 결과 예상 가용은 86" IFP 18 → −46, STD1 17 → −8,
  75" IFP 1 → −14, T1 30 → −1 이다. 원인은 날짜 없는 수주 대기(갈무리국어 86" 40대 등)가 예정으로 들어오기 때문이다.
- **시트 쪽 사람 조치**:
  - 재고현황 86" IFP 입고 "합계" 수식 범위에 C2 열을 넣는다.
  - 2025-02~03 날짜의 로트 지정 "배송 예정" 10행(같은 물류No·제품의 설치 완료 행이 모두 있음)의 진행 상태를 정리한다.
- **어드민 확정 × 교체 가져오기 이중 계상**(3-6 미해결): "시트가 이긴다" 결정 후 교체 RPC 마이그레이션.
- **미적용 마이그레이션 3종**(6절): 금액 컬럼·복원 가드.
- **전시 상태 마이그레이션 적용**(6절 20260915) 후 샘플 유닛 33대 정리: 쇼룸·KC인증 3대 → 전시, 고객 미상 16대 → 실사 후 사무실 보관·대여 정정.
- **입고표를 쓰기 전에 시트 가져오기부터**: 원장에 C2가 없어 새 물량 추천이 C2로 뜬다(안내 문구는 붙였다).
- **cutover 실행**(위 런북 2~5: 배포→백필→플래그 on→패리티). 현재 플래그 off로 동작 무변.
- 읽기 경로 최적화(투영/캐시), 스냅샷 retention.
- 제품 사진 칩 승격, 출고 매출(revenue) 필드, 잔여 디자인 정규화(전 입력 2px 포커스 링·타입/라디우스 통일·헤더 단일화).

...
- 어드민 화면: `/admin/docs/gaps` ([app/admin/docs/gaps/page.tsx](../../app/admin/docs/gaps/page.tsx)) — 사이드바 미등록(URL 접근)
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 임베딩 모델 | `gemini-embedding-001`, **768차원** | 한국어 검색 품질 우선 (트래픽 98% 한국). 컬럼 `vector(768)`과 정합 |
...
| AI 초안 | **자동 게시 금지** | 항상 어드민 검토 게이트. 생성만 하고 저장 안 함 |
...
- [ ] **0. 프로드 env 확인**
  ...
  - [ ] `supabase/migrations/20260421_docs_center.sql` 적용 상태(문서센터 테이블)
...
- [ ] **2. 벡터 검색 마이그레이션 적용**
  ...
  - [ ] [20260613_docs_chunk_embedding_768.sql](../../supabase/migrations/20260613_docs_chunk_embedding_768.sql) (768 정렬 — 최종)
  - 참고: 768 쪽만 적용해도 멱등 동작(`drop … if exists`)

- [ ] **3. 임베딩 백필** (gemini-embedding-001, 768d)
...
- 어드민 사이드바에 `/admin/docs/gaps` 링크 등록
...
| AI 초안 생성 | `POST /api/admin/docs/gaps/draft` (저장 안 함) |
| 검색 RPC | `match_docs_ai_chunks(vector(768), int)` |

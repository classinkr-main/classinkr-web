...
두 저장소의 `vercel.json`은 `icn1`로 수정하고, 검증된 운영 커밋을 기준으로 별도 이관 브랜치에 기록한다. **운영 브랜치에는 아직 반영되지 않았다.** 다음 Git 배포에 이 변경을 반드시 포함해야 한다. 현재 원격 운영 브랜치의 `sin1` 설정을 그대로 배포하면 Vercel 기본 리전보다 우선하여 실행 위치가 되돌아갈 수 있다. 이번 운영 전환에서는 기존 Production 코드만 배포했으며, 별도의 미배포 커밋이나 작업 중 변경을 함께 배포하지 않았다. (2026-09-14 갱신: 홈페이지 저장소의 `vercel.json` `icn1` 변경은 같은 날 운영 브랜치에 반영됐다. 이후 병합하는 오래된 작업 줄기가 `sin1`을 되살리지 않는지만 확인하면 된다.)
...
## 로컬 개발 환경 점검 (2026-09-21 추가)

1. 이관은 한 작업 머신에서 수행됐으므로 다른 머신의 `.env.local`은 이관 전 값일 수 있다 — `NEXT_PUBLIC_SUPABASE_URL`의 호스트가 위 표의 "현재 운영 프로젝트" ref 인지 확인한다.
2. 이관 전 값이면 DB를 읽는 로컬 작업(`check:db`, `check:alpha-db`, `npm run build`의 postbuild `check:public-content`, dev 서버의 어드민 화면)이 `fetch failed`로 실패한다 — 2026-09-21 실측에서 기존 프로젝트 호스트는 DNS 조회(ENOTFOUND)부터 실패했고 서울 호스트는 정상 응답했다. 위 "원본 보관" 절의 "읽기와 관리 접근은 유지한다"와 다른 관측이므로 기존 프로젝트의 현재 상태(일시정지 여부)를 대시보드에서 확인한다.
3. 서울 프로젝트의 URL·publishable key·secret key로 교체하고, 키 값은 문서·로그·커밋에 남기지 않는다.

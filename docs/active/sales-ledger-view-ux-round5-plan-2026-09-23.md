...
구현을 끝낸 뒤 변경 전체를 따로 검토해 실제 결함 9건을 찾았고, 모두 고쳤다(회귀 고정: `tests/branch/round5-review-fixes.test.ts`).

| # | 결함 | 수리 |
| --- | --- | --- |
| 1 | R-12 0 차단이 주차 병합 행의 "한 주만 0"(합계는 양수)까지 막았다 | 차단 기준을 "만들어질 초안 금액 ≤ 0"으로 옮김(`onCommitCell`) |
| 2 | 서버 시드로 연 화면은 첫 새로고침·팀 전환에서 직전 값이 없어 매트릭스가 깜빡이고 콕핏이 다시 마운트됐다 | 마지막으로 보인 행을 기억해 로딩 중 "갱신 중"으로 유지 |
| 3 | 콕핏 월 동기화가 레일에서 친 값을 몰랐고, 저장 중 친 값을 지웠고, 같은 달 선택이 무반응, 열린 큐를 닫았다 | 레일·콕핏 공통 편집 순번, 저장 시점 순번으로만 깨끗하게, 편집기 월 선택은 바로 재로드, `keepRail` |
| 4 | 대상 해제·초안 편집·새 딜 직후 늦게 온 상세 응답이 폼에 이전 딜 고객·담당을 되채웠다 | 폼 소유 순번(`draftFormOwnerRef`), 대상 해제는 상세 요청도 끊음 |
| 5 | 한 칸 붙여넣기가 방금 잠긴 칸(남은 선택)에도 편집으로 들어갔다 | 편집 가능한 칸일 때만 |
| 6 | 콕핏 선택 딜의 "새 딜" 저장 결과가 편집기 재마운트로 사라졌다 | 그 경우 결과를 토스트로 |
| 7 | 기존 딜 칸에 고객명으로 걸린 **신규** 초안을 칸 값으로 보였다(R-2) | 더해지는 초안은 표시만("신규 +¥…"), 칸 값·복사는 장부 값 |
| 8 | 오류 토스트가 남게 된 뒤로 오류 3건이면 새 info(실행 취소 등)가 즉시 밀렸다, 모바일 레일 시트를 가렸다 | 기존 것 중에서만 밀기, 시트가 열리면 위쪽에 |
| 9 | 편집 바 미리보기가 "1234.5"를 ¥12,345로 보였다(저장은 ¥1,235) | 커밋과 같은 파싱(`previewMatrixAmount`) |

7번을 따라가다 **라운드 5 이전부터 있던 정합 결함**도 함께 막았다: 같은 칸에 신규 초안이 걸려 있으면 기존 딜 칸의 재편집·레일
수정 저장이 그 신규 초안을 갱신 대상으로 잡아 PATCH했다(신규 초안이 수정 값으로 바뀌거나 이중 계상). 이제 편집 대상 판정은 초안
종류가 맞는 것만 보고(`lookupMatrixPending(…, kind)`·`railDedupTarget`), 같은 칸이면 수정 초안이 신규 초안보다 앞선다.

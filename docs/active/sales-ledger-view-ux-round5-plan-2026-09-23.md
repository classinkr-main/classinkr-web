...
- **REV CSV**: 현재 필터·정렬이 반영된 **전체** 행(페이지 무관) × FY 12개월, 원 단위 정수, 행마다 확정·고확도·예정 분해 열. UTF-8 BOM(엑셀 한글 호환),
  파일명 `매출장부_REV_{FY}_{팀}_{내려받은 날짜}.csv`. 미적용 초안은 넣지 않는다(장부 기준 값만). 이 기준은 버튼 title과 완료 토스트로
  말한다 — 구현하며 파일 머리 설명 줄은 뺐다(첫 줄이 머리글이 아니면 엑셀 필터·피벗이 깨진다).
...
- REV: 대기 초안 금액을 셀에 표시(R-2, 결정 E1), 셀 title·편집 바에 원 단위(R-8), 검색창 Enter/↓ → 격자(R-7),
  0 커밋 선차단(R-12), 1칸 붙여넣기 = 편집 채움(R-13), 잠긴 셀 Enter → 상세 + 실제로 고치는 경로 안내(R-14 — 레일은 잠긴 달
  수정 초안을 막으므로 "레일 정정 입력"에서 바꿨다, §5.1).

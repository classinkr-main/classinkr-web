...
- **CRM 홈 목록 예산(2026-08-05)**: 홈은 요약 표면이라 목록 패널이 스크롤 목록이 되면 안 된다. 첫 화면 본문 행 수는 **작업대 6 + 주간 할 일 6 + 설치·방문 3 = 15행**이 상한이고, 넘치는 만큼은 전부 "+N건 더 보기"로 접는다(펼침은 이미 받아온 배열을 쓰므로 추가 요청 없음). 특히 `CrmWeekAheadPanel`은 `limit=100`으로 받아 5개 버킷을 **전부** 그리던 자리라 활성 할 일이 많으면 그대로 수십 행이 됐다 — `budgetWeekAheadBuckets`(지연 → 오늘 → 이번 주 순 예산 배분)를 거치게 하고, 버킷 헤더 카운트는 잘라내기 **전** 총량을 쓴다. 홈에 목록 패널을 추가할 때 이 예산을 지키고, 상한을 넘긴 건수는 반드시 화면에 숫자로 남긴다(조용히 자르지 않는다).
- **CRM 홈 표면 규칙(2026-08-05)**: `/admin/crm`은 행동면(작업대·고객 찾기·리드 요약·주간 조망)과 참조면(리포트)으로만 나뉜다. **참조·보고 블록은 최상위에 새로 쌓지 말고 `CrmHomeReportSection`의 탭에 추가한다** — 기본 접힘 + 선택 탭만 렌더라 Recharts 청크가 첫 페인트에서 빠진다. 같은 데이터를 두 표면에 두지 않는다: 설치·방문은 일정 카드 한 곳, 할 일은 `CrmWeekAheadPanel` 한 곳(액션 레일은 `hideTasks`로 끈다 — 레일의 '오늘 할 일'은 주간 패널 '오늘' 버킷의 부분집합), 고객 진입(검색·최근 본·자주 접촉)은 '고객 찾기' 한 곳, 화면 이동 딥링크는 하단 '바로 가기' 한 줄.

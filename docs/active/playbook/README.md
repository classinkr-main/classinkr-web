...
2. **담당 파트 위임 (현재 수동)**: 전담 서브에이전트(`.claude/agents/`)는 아직 미구현이다(계획). 현재는 사람이 파트를 판별해 아래 3~5단계를 직접 수행한다. 구현되면 이 단계가 `Agent(subagent_type: "chatbot")` 식 자동 위임으로 승격된다.
...
- 파트 구조·철칙이 바뀌면 이 README와 해당 파트 가이드를 **함께** 갱신한다. (전담 에이전트 `.claude/agents/`를 실제 구축한 경우, 대응 정의도 같이 갱신한다.)
- 각 가이드의 "현재 목표 & 백로그"는 스냅샷이다. 큰 사이클이 끝나면 날짜와 함께 갱신한다.
- 새 파트가 생기면: 가이드 1개 + 이 README의 §1·§2 행 추가. (에이전트를 구축한다면 대응 정의 1개도 함께.)

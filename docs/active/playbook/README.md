...
2. **담당 에이전트 위임**: 해당 파트의 전담 에이전트를 띄운다. 예) 챗봇 작업 → `Agent(subagent_type: "chatbot")`. 각 에이전트는 자기 파트 가이드 + `work-flow-patterns.md`(공통 함정·체크리스트)를 SSOT로 참조하는 얇은 정의다(정의: `.claude/agents/`). 위임 없이 사람이 직접 할 경우 아래 3단계로 가이드를 정독한다.
...
- 파트 구조·철칙이 바뀌면 이 README와 해당 파트 가이드, 그리고 `.claude/agents/`의 대응 에이전트 정의를 **함께** 갱신한다. 에이전트는 가이드를 가리키는 얇은 정의이므로 사실은 가이드에서 먼저 고친다.
- 각 가이드의 "현재 목표 & 백로그"는 스냅샷이다. 큰 사이클이 끝나면 날짜와 함께 갱신한다.
- 새 파트가 생기면: 가이드 1개 + `.claude/agents/` 에이전트 1개 + 이 README의 §1·§2 행 추가.

...
- **KB 활성화 상태 (2026-08-05 최신화)**: 코드 루프 완결. 채널톡 동기화(`scripts/sync-channel-documents.ts`)로 54개 아티클 297개 청크가 `docs_ai_chunks`에 적재되었으며, 임베딩 백필(`scripts/embed-docs-chunks.ts`) 297/297건 100% 가동되어 **하이브리드 벡터 검색 활성화 중**. `npm run check:alpha-db` 파이프라인 계약 검증 `Status: ok`. 운영 의존: prod `GEMINI_API_KEY` 미설정이면 임베딩→키워드 폴백·답변→템플릿 폴백.

...
### §13 Compass 정리 라운드 — 2026-09-20 완료 (S1–S4 · D1–D3 · Q1–Q4)

§13.2 의 11개 항목을 스키마 변경 없이 구현했다. 공용 계약 2종(`lib/crm/lead-segments.ts` 세그먼트 SSOT, `lib/compass/summary-contract.ts` 요약 응답 타입)을 먼저 커밋하고, 하위 모델(Sonnet) 서브에이전트 7개가 파일 소유권이 겹치지 않는 7개 클러스터(리드 보드 세그먼트·검색 / 팔레트·통합 저장 뷰 / 요약 API / 홈 밴드 / 인사이트 섹션 / 리드 드로어 칩 / 360 할 일·태그 칩)를 병렬 구현한 뒤 클러스터별로 검증·커밋했다. 조사 단계도 Haiku 탐색 에이전트 3개로 나눠 돌렸다.

핵심 반영 사항:

- S1·S2: 리드 보드 상단에 세그먼트 칩(전체·메타 광고·인계·기존·고객)과 `?segment=` URL 축이 생겼다. 칩 건수는 세그먼트만 뺀 나머지 필터 기준이고, Compass 가 끊기면 인계·기존 칩은 0 이 아니라 "연결 끊김"으로 비활성되며 선택 중이던 세그먼트는 인라인 캡션과 함께 전체로 통과시킨다. 검색은 Compass 매칭 리드의 학원명·이름까지 본다(`lead-ranking` haystack `extraTerms`). "/" 포커스에 IME 가드.
- S3·S4: 커맨드 팔레트에 세그먼트 이동 명령 4개. 통합 고객에 저장 뷰 "메타 광고 리드"(origin=ad)·"NEO 등록 리드"(crmRegistered)를 추가하고 서버 viewCounts·라우트 화이트리스트까지 연결했다.
- D1: `GET /api/admin/crm/compass-summary?period=7d|30d|90d`. 브리지에 금액 컬럼을 제외한 range 슬라이스 조회(1,000행 페이지, 상한 5,000 → truncated)를 추가했고, 기간 내 유입·플랫폼(메타 정규화)·단계 누적 퍼널·이탈·NeoCRM 등록·결제·케어 사다리·담당별·이탈 사유·다음 액션 임박 목록·BD인계·오늘 데모를 한 응답으로 준다. 역할 매트릭스 테스트 갱신(65 핸들러).
- D2: 홈 Compass 밴드가 숫자 3개 위에 기간 칩, 세그먼트 타일 4개(리드 보드 딥링크), MiniFunnel 단계 퍼널, 유입 플랫폼 막대, 다음 액션 임박 8행(새 탭 딥링크)을 보여 준다. down 이면 요약 블록 전체를 "연결 끊김" 한 줄로 강등한다.
- D3: 인사이트에 "Compass 파이프라인" 섹션 — 담당별 진행 표(유입·데모·BD인계·결제·이탈), 케어 5단계, 플랫폼 상위 6, 이탈 사유 상위 5. 인사이트의 로컬 90초 TTL 도 SSOT 로 정리했다.
- Q1·Q4: 리드 드로어에 팔로업 프리셋 칩(오늘·내일·3일 뒤·다음 주 월·지우기) 즉시 저장, 부재중/재통화 저장 직후 "팔로업 내일/3일 뒤" 제안 칩 8초, "나"+최근 배정 3명 빠른 배정 칩(localStorage).
- Q2·Q3: 고객 360 할 일 탭에 빠른 추가 칩 4개(낙관 추가 + 8초 되돌리기), 개요에 태그 칩 편집기(제안 태그 원클릭·직접 입력·× 제거) — 태그 UI 최초 도입.

검증: `npm run typecheck` 통과, 이번 라운드 변경 파일 전체 `eslint --max-warnings=0` 통과, `npx vitest run` 621 파일 / 4,836 케이스 통과, `npm run build` 통과.

후속으로 남긴 것:

- 세그먼트 정의는 SSOT 한 파일에 있다. "기존 리드"를 Compass 매칭 전체로 볼지 NeoCRM 등록만으로 좁힐지는 운영 확인 후 `resolveLeadSegments` 만 바꾸면 된다.
- 요약 API 의 byOwner 는 total 상위 8 을 자른 뒤 그 안에서 미배정을 맨 뒤로 보낸다 — 담당이 8명을 넘으면 미배정 행이 빠질 수 있다. lostReasons 는 stage=lost 행만 센다.
- 세그먼트 칩 위치는 "필터 카운트 카드 위"다(검색창이 유입 칩 박스 안에 있어 "검색창 아래"와 동시에 만족할 수 없었다).
- 홈 밴드의 세그먼트 타일은 공용 StatTile bare 변형을 재사용해 내부 구분선이 #f0f0ec 로 남는다. 인사이트 Compass 섹션은 마스터 새로고침 버튼과 연결하지 않았다(자체 신선도 캡션으로 재조회).
- 할 일 빠른 추가는 360 클라이언트가 customerKey·고객 이름을 넘기며, 라우트 파라미터는 폴백이다. 담당 select 실패 시 값 자동 되돌림은 하지 않는 기존 규약(캡션+재시도)을 칩도 따른다.
- DOM 테스트 환경이 없어 칩 클릭 상호작용은 순수 함수 + 정적 마크업 + 소스 계약 테스트로 고정했다.

### §11 2단계 — 2026-09-21 완료 (M2·M4·A1·A4·T1·T4·P1·P3)

§11.5 2단계 8개 항목을 §14 의 6개 클러스터로 나눠 Sonnet 서브에이전트가 병렬 구현했다(조사는 Haiku 3건). 스키마 변경 없음 — 출고→NEO 계정 링크는 `crm_source_links.source_system="branch_hw"` 로 기존 테이블에 얹었다.

핵심 반영 사항:

- M2·M4: 360 매출 탭에 품목별 대수 표(딜 라인아이템·HW 출고를 품목·통화·근거·출처별로 합산, 통화 간 합산 없음, 확정/추정 배지, 행 펼침으로 시리얼·주문번호)와 "연결 대기 출고" 섹션(이름 유사 후보 → `POST /api/admin/crm/source-links/hw-outbound` 로 확정 링크 → 낙관 반영 후 `router.refresh` 재조립). NEO 미동기화 계정도 HW·딜만 있으면 표가 보인다.
- A1: 회의록 모드 필수 필드를 요지 한 줄로 줄이고(결정·차단·참석자는 +상세), 컴포저에 최근 4 + 오늘 연락 4 고객 칩(오늘 우선, 최대 6) 원클릭 연결.
- A4·P1a: 기록 목록이 KST 날짜 그룹(스티키 헤더·건수)으로 바뀌고 기간 칩(오늘·7일·30일·전체)·분위기 칩이 생겼다(서버 from/to 파라미터 추가, 분위기 select 제거). 기록 화면에 홈과 같은 스트리밍 프리페치 레인 + 캐시 시드(URL 조립 SSOT `lib/crm/activity-events-url.ts`).
- T1: 인사이트의 리드 3단계 블록을 리드 상태 → 딜 7단계 9단계 퍼널로 대체(리드는 순차 누적, 딜은 현재 단계 이상 누적, lost·closed 별도, 전환율 최저 구간 병목 배지). 딜 단계 집계 실패면 반쪽 퍼널 대신 EmptyState.
- T4: `/admin/crm/customers/tags` 태그 관리 화면(서브내비 "태그") — 전체 태그·건수·리드/NEO/고객 분해, 이름 변경·병합을 dryRun 미리보기 후 인라인 확인으로 일괄 반영.
- P1b·P3: 통합 고객 화면 스트리밍 프리페치 레인(클라 기본 URL 과 동일한 시드 URL). 고객 360 은 방문한 탭을 hidden 으로 유지해 펼침·더 보기 같은 로컬 상태가 탭 전환에도 남는다.
- 역할 매트릭스 테스트에 새 라우트 2개(hw-outbound POST, tags GET/PATCH) 등록 — 68 핸들러.

검증: `npm run typecheck` 통과, 2단계 변경 파일 전체 `eslint --max-warnings=0` 통과, `npx vitest run` 639 파일 / 5,026 케이스 통과, `npm run build` 통과.

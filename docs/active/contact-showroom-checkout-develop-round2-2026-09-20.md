...
**해소(2026-09-22).** 운영 방침이 "공휴일은 쉰다"로 확정됐다. 법정 공휴일은 고정 목록([lib/korea-public-holidays.ts](../../lib/korea-public-holidays.ts), 2026~2027 월력요항 대조)이 하한선이 되고, 구글 원천은 그 위에 임시공휴일만 더한다([lib/korea-holiday-dates.ts](../../lib/korea-holiday-dates.ts)). 쇼룸 가용성과 `/checkout` 희망일이 같은 함수를 쓰므로 두 화면이 함께 막힌다. 자격이 없어도 설·추석은 막히고, 빠지는 것은 임시공휴일뿐이다. 고정 목록은 매년 6월 무렵 다음 해 월력요항이 나오면 한 해치를 더한다. 목록이 희망일 창(+180일)보다 짧아지면 테스트가 실패하고 운영 로그에 경고가 남는다.
...
| B3 | **고객 확인 경로** — 접수 확인을 연락처로 보낸다. 알림톡 템플릿 승인 전까지는 문자, 템플릿 ID 환경변수가 채워지면 알림톡 | P1 |
| B4 | **가용성 에러 시 재시도** — 폼이 영구 비활성으로 잠기지 않게 | P1 |
| B5 | **원천 degradation 신호** — 공휴일·ICS 원천이 비었을 때 운영 로그에 드러낸다. **적용(2026-09-22)**: 공휴일은 방침을 고정 목록으로 강제하므로(§3-2 S9 해소) 고객 화면 안내는 두지 않는다. Phase B 에 넣었던 안내 문구는 클라이언트가 원천 신호를 받지 않아 한 번도 켜진 적이 없었다 — 문구와 공개 응답의 원천 필드를 함께 걷었다 | P1 |
...
| C6 | **`/checkout` 계측 복구** — 광고 전환 발화는 적용(2026-09-21). GTM·PageViewTracker·동의 게이트 픽셀(Meta·네이버)·동의 배너도 **적용(2026-09-22, §8 D11)**. 헤더·푸터·챗봇·채널톡은 계속 걷는다. 크롬과 계측의 경로 판정은 [lib/analytics-paths.ts](../../lib/analytics-paths.ts) 한 곳에 둔다 | P2 |
...
| D6 | **공개 화면 디자인 가드** — `SCAN_ROOTS` 확대 + `slate-`·`font-serif` 패턴 추가 | §7-2 |
| D7 | **폼 계약 테스트** — 문의 폼 필드 매핑, 캘린더·슬롯 상호작용 | §9 |

**Phase D 적용(2026-09-22).** 문의 화면 정리와 가드 확대는 짝이다 — 가드를 먼저 넓히면 기존
위반 때문에 빌드가 깨진다. `app/contact` 를 웜 뉴트럴로 옮긴 뒤 가드를 넓혔고, 퍼널 7개 루트
69개 파일이 `prebuild` 에서 검사된다. 나머지 공개 화면(`components/sections/*` 등)은 아직
범위 밖이다 — 한 번에 넓히면 무관한 기존 위반이 쏟아져 게이트가 무력해진다.

파일 분리는 **표현 전용 블록 두 개**(빠른 상담 배너, 직접 연락 경로)만 떼어 750줄 → 605줄로
줄였다. 폼 본체는 상태가 얽혀 있어 그대로 뒀다 — 쪼개는 값어치보다 회귀 위험이 크고, 원래
문제(위반이 숨는다)는 가드와 마크업 테스트가 이미 막는다.
...
- **저하**: 공휴일·ICS 원천이 비어 있을 때 화면은 정상 동작하되(방침 유지), 운영 로그에 남긴다. 자격 미설정은 일시 장애와 달리 **상시 상태**이므로 배포 전 점검 항목으로도 둔다. 법정 공휴일은 고정 목록이라 이 저하와 무관하게 막힌다(§3-2 S9 해소).
...
| 항목 | 내용 |
|---|---|
| 확인 발송 | **적용(2026-09-21).** 접수 확인을 연락처로 보낸다. `phone` 이 두 폼 모두 필수라 커버리지 100%. 발송 실패는 접수를 되돌리지 않고 `message_logs` 에 남는다 |
| 알림톡 전환 | 템플릿은 카카오 비즈니스 채널 사전 승인이 필요하다. 승인 뒤 `SOLAPI_TEMPLATE_*` 환경변수만 채우면 코드 변경 없이 알림톡으로 바뀐다 |
| 상태 가시성 | `new`/`contacted`/`scheduled`는 내부 값이다. 공개 조회를 만들 경우 노출 어휘를 따로 정의한다 — 아직 미정 |
...
| D10 | **고객 확인 발송 범위** | **확정(2026-09-21): 문자 · 알림톡(solapi).** 이메일은 쓰지 않는다 | 운영 결정. 이메일 제안은 **틀린 전제였다** — `phone` 이 두 폼 모두 **필수** 항목이라 문자는 커버리지가 100% 이고 폼 마찰이 0 이다. 이메일은 양쪽 다 선택 필드여서 받은 사람에게만 보낼 수 있었다. solapi 발송 레이어([lib/messaging/send.ts](../../lib/messaging/send.ts))가 이미 운영 중이라 새 발송 수단을 만들 필요도 없었다 |
| D11 | **`/checkout` 에 GTM·PageViewTracker·Pixel·동의 배너를 되살릴지** | **확정(2026-09-22): 되살린다.** 크롬(헤더·푸터·챗봇·채널톡)은 계속 걷는다. `/receipt`·`/admin`은 계속 계측하지 않는다 | 내부 적재는 분석 동의가 있을 때만 일어난다([lib/analytics.ts](../../lib/analytics.ts)). 그래서 배너를 뺀 채로는 광고 딥링크로 직행한 방문자가 **동의할 기회조차 없어** 퍼널 마지막 단계가 데이터에서 사라졌다. 직행 방문자의 내부 적재는 공개 화면과 같은 옵트인 규칙을 따른다. 배너는 닫기 버튼이 있는 하단 카드이고, `/checkout`에는 다른 하단 고정 요소가 없어 서로 부딪히지 않는다. Meta Pixel·네이버 로그분석 컴포넌트가 따로 들고 있던 `/checkout` 차단도 같은 판정으로 합치고, 복사본이 다시 생기지 않게 소스 검사 테스트를 걸었다 |
...
**공휴일 자격 미설정(§3-2 S9)은 결정 사항이 아니라 운영 점검 항목이다.** 운영 방침("공휴일은 쉰다")은 고정 목록이 강제하므로, 자격이 없어도 법정 공휴일은 막힌다. 자격은 임시공휴일 반영에만 걸린다. `GOOGLE_SERVICE_ACCOUNT_EMAIL`/`GOOGLE_PRIVATE_KEY` 설정 여부는 배포 전 점검 항목으로 남긴다. 고정 목록이 들어가기 전 배포본에서 자격이 없었다면 그동안 연휴 예약을 받고 있었다는 뜻이므로, 그 기간의 쇼룸 접수를 한 번 훑어본다.
...
| 공휴일 고정 목록·원천 병합 | `npx vitest run tests/lib/korea-public-holidays.test.ts` — 매년 6월 월력요항 발표 후 한 해치를 더한다 |
| 크롬·계측 경로 범위 | `npx vitest run tests/lib/analytics-paths.test.ts` |

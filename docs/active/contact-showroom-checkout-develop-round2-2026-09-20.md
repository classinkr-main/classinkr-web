...
| C6 | **`/checkout` 계측 복구** — 광고 전환 발화는 적용(2026-09-21). Consent Mode v2 가 gtag 층에서 게이트하므로 동의 배너와 무관하다. GTM·PageViewTracker·Pixel·동의 배너 복구는 §8 D11 결정 후 | P2 |
...
| D11 | **`/checkout` 에 GTM·PageViewTracker·Pixel·동의 배너를 되살릴지** | **미결** | 광고 전환 발화는 이미 분리해 적용했다(gtag 는 `/checkout` 에도 로드되고 Consent Mode v2 가 발화를 게이트한다). 남은 것은 `page_view`·Pixel·동의 배너인데, 배너를 띄우면 헤더·푸터를 걷어낸 터널 UX 가 바뀐다. 동의 없이 직행한 방문자의 내부 적재를 어떻게 다룰지와 함께 정해야 한다 |

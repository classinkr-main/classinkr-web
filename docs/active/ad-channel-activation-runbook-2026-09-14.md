...
| `truncated: true` | 조회 범위가 92일 상한을 넘었다 | 크론(trailing 3일)에서는 날 수 없다. 백필을 돌렸다면 범위를 나눠 다시 돌린다 |

> `/stats` 요청 인코딩(`ids` 반복 파라미터 · `fields`/`timeRange` JSON **문자열**)은
> 공개된 동작 예제와 대조해 확인했다. 그래도 틀린다면 **400 으로 시끄럽게** 죽으므로
> 조용한 오답은 나지 않는다 — 에러 본문에 네이버가 준 사유가 그대로 실린다.

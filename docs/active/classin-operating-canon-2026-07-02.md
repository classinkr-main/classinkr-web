...
**표준 5단계:** (1) 소유권 매트릭스로 파트 판별 → (2) 전담 에이전트(`.claude/agents/`) 위임 *(주 2026-07-09: 에이전트 미구현 — 현재는 파트 가이드 직접 정독으로 대체)* → (3) 파트 가이드(`docs/active/playbook/0N-*.md`) 정독 → (4) 공통 철칙 7개 적용 → (5) 검증. 크로스컷 의존성(콘텐츠↔챗봇 KB, 그로스↔플랫폼 스키마, 프론트↔그로스 계측·동의, 어드민이 3·4·5 화면 호스팅) 확인.

...
## 0. 먼저 — 클래스인을 이해한다 (진입점)
이 캐논은 *카피 규칙집*이 아니라 **에이전트가 클래스인을 사업 수준으로 이해하기 위한 기준**이다. 페이지·카피 규칙은 이 이해에서 *파생*된다.
- **[what-is-classin.md](./what-is-classin.md) — 클래스인 이해 기준.** 가이드·챗봇 문서(`lib/docs.ts`)·리드마그넷(`lead-magnets.json`)에서 역추출한 제품 실체·고객 페인(빈도순)·영업 모션·반론 처리·정직한 한계·ICP·지뢰밭. 새 작업 전 먼저 읽는다.

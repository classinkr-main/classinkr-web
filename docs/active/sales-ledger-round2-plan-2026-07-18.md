...
- [ ] **주차별 확정/예상 개별 기록** — P1, 규모 大
  `metadata.weekly`를 `[금액,'c'|'e']` 5칸으로 확장(시안 데이터 모델과 동형). 초안 적용·확도 맵 분해
  (revenue-core)·주간 마감 버킷까지 파급되는 스키마 변경이라 별도 설계 필요. **선행 조건**: 아래 항목들
  구현 전에 이 스키마부터 결정해야 재작업이 없다(2·3번이 이 확장을 전제로 하는 표시 항목).
- [ ] **분기/월 확도 스택 게이지를 REV 카드 헤더로 승격** — P2, 규모 小
  보드 헤더의 확정/고확도/예정(빗금) 바를 REV 렌즈 상단에도. 이미 있는 요약 타일과 중복 여부 판단 먼저.
- [ ] **매트릭스 셀 확도 표현을 형태 문법으로 보강** — P2, 규모 中
  현재 색·아이콘 기반에 "예정=점선 테두리" 추가 여부. 밀도(12개월×수백 행) 렌더 비용과 시각 소음 검토 후.
- [ ] **"내 딜" 스코프 프리셋** — P3, 규모 小
  담당자 다중필터 위에 세션 사용자 기본 핀(KR Team 개요의 pinned manager 규약 재사용).
- [ ] **Cockpit-1c 마스터-디테일 입력 콕핏 재평가** — P3, 규모 大(보류 성향)
  좌 딜 리스트+우 에디터 전용 화면. 현 구조(매트릭스+레일)와 중복이 커서, 위 항목들이 안착한 뒤
  필요성 재평가 — 결론 없이 착수 금지.
- [ ] **Customers 탭** — P4, 보류

...
## 15. 2026-08-18 후속 결정 — 딥링크 근본 수리 · 대시보드 행동 허브화

### 15.1 `tab` 딥링크의 pushState 구멍을 훅 수준에서 봉합

§5가 기록한 구멍 — "Next `<Link>`의 pushState는 popstate를 쏘지 않아 `useUrlState`가 주소
변경을 못 본다" — 이 실제로 콘솔 이동을 끊고 있었다. 같은 경로 안에서 쿼리만 바뀌는
콘솔 메뉴 이동(미해결 큐 ↔ 가이드 문서 ↔ AI 품질 검수 ↔ 추천 질문, 전부 `/admin/docs?tab=…`)
에서 내비 활성 표시는 바뀌는데 본문은 직전 탭에 멈추는 것을 실측했다.

수리는 화면별 보정이 아니라 [`useUrlState`](../../lib/use-url-state.ts) 자체에 했다 —
`history.pushState/replaceState`를 1회 감싸 `URL_STATE_CHANGE_EVENT`를 쏘고 구독이 popstate와
함께 듣는다(계약 테스트 tests/lib/use-url-state.test.ts). 이로써:

- §5의 "합성 읽기(`searchParams.get("tab") ?? tabParam`)" 계획은 **불필요해졌다**. 합성이
  경계하던 재렌더 지연 문제 없이 라우터 커밋 즉시 훅이 새 값을 읽는다.
- 상담 Inbox의 "라우터 커밋 신호" 보정(§5)은 무해한 이중 알림이 되어 그대로 둔다.
- 남은 한계 하나: 하드 로드 딥링크는 SSR이 기본 탭을 그렸다가 하이드레이션 후 교정된다
  (짧은 플래시). `useSearchParams` 없이는 제거할 수 없어 수용한다.

### 15.2 외부 대시보드 = 지표 + 행동 허브 (§7 유지)

`/admin/chatbot`은 "순수 대시보드"에서 한 발 나아가, 숫자가 행동으로 이어지는 허브로 확장했다.

- 지표 6카드는 각각 그 숫자를 처리하는 화면으로 가는 딥링크다(질문량→질문 패턴,
  미해결률→미해결 큐 `?source=chatbot`, 상담 이관·채널톡 전송→상담 Inbox,
  응답 속도·평균 신뢰도→AI 품질 검수). 목적지 라벨을 카드에 상시 노출한다.
- `미해결 최신 5건` 읽기 전용 프리뷰를 추가했다(`/api/admin/docs/gaps?limit=5`).
  행 클릭은 큐로 이동만 한다 — 초안 생성·게시 같은 처리 액션은 여전히 미해결 큐에만
  있으므로 §7의 진입점 단일화는 유지된다.
- 지표 로딩은 스켈레톤, 실패는 "—"+다시 시도로 분리한다(로딩·실패·0값을 섞지 않는다).

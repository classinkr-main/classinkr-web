...
| 이중 계상 정책(reference §8-6) | **(a) "가져오기 때는 시트가 이긴다" — 2026-09-21 결정·구현 완료** | P2-1 착수 조건이 풀렸다 |
...
**P0-3. 행별 확정 재검증 — 하지 않기로 했다 (2026-09-20 검토)**
- 행별 확정의 `await refresh()`는 낭비가 아니라 **잠금**이다. `plannedConfirmLocked`가 확정 중 다른 확정 경로를 모두 잠그는 이유가 "순차 실행 중간에 다른 확정이 끼어들면 FIFO 배정이 로트 잔량을 놓고 경쟁한다"이므로, 잔량을 다시 받기 전에 잠금을 풀면 그 보호가 사라진다.
- 여러 건을 빠르게 확정하는 길은 이미 있다 — **선택 일괄 확정**(공통 확정일, 재검증 1회). S4 목표는 그쪽으로 달성한다.
...
**P1-4. 기억값 확장 — 하지 않기로 했다 (2026-09-20 검토)**
- 담당자는 이미 기억한다. 남은 후보(출발지·보관처·수입자)는 **경로를 정하는 값**이라, 사람별 기억값이 프리셋 기본값을 조용히 덮으면 출발지가 틀린 기록이 원장에 남는다. 아끼는 타건 수보다 위험이 크다.
- 입고의 보관처·수입자·단가는 이미 최근 원장에서 추천한다(`inbound-sheet-model.ts`의 `recentInboundStorage`·`recentInboundImporter`·`lookupLastUnitPrice`) — 사람별 기억이 아니라 데이터가 채운다. 그 방향을 유지한다.

**P1-5. 키보드**
- 목록에서 `i` = 입고표, `o` = 출고 시트(입력 포커스·모달·IME 조합·수정 키 조합 중에는 무시). 시트에서 Cmd/Ctrl+Enter 저장 — 입고표와 같은 규약(`inbound-sheet-model.ts:929`).

### P2 — 입력 자체를 줄이기 (2026-09-21 결정 후 착수, §9 실행 기록)
...
- **P3-1. 드래프트 자동 저장·복구** — 구현 완료(§9 실행 기록).
...
- **P0-1 저장 후 낙관 반영 + 백그라운드 재검증** — 단건 저장·바구니 저장이 서버가 돌려준 원장 줄을 즉시 화면에 끼워 넣고(`applySavedMovements`), 재검증은 기다리지 않는다. 저장 버튼 잠금이 요청 시간만큼으로 줄어 연속 기록에서 다음 건을 바로 받는다.
  - 재고·가용·알림 등 **파생 숫자는 손대지 않는다**(서버 계산이 정본). 정렬 키는 서버와 같은 `occurred_at ?? created_at` 내림차순이라 어제 날짜 기록이 맨 위로 튀지 않는다.
  - 재검증이 겹칠 수 있게 되어 `load()`에 응답 순번 가드를 넣었다 — 늦게 온 옛 응답이 새 응답을 덮어쓰지 않는다.
  - 취소(void)·기록 수정은 그대로 재검증을 기다린다. 자주 하는 입력 루프가 아니고, 화면의 그 행이 바뀌는 것을 보는 편이 안전하다.
- **P0-2 배치 저장 1왕복** — 라우트가 줄마다 저장소를 부르던 루프를 `createHardwareMovementsBatch` 한 번으로 바꿨다. 줄별 결과(201/207/422·`lineResults`) 계약은 그대로다.
  - 품목 해결 1회 · CRM 중복 조회 1회 · 품목 원장 읽기 **품목당 1회** · INSERT 1회 · 캐시 무효화 1회.
  - 로트 배정은 줄 단위 저장과 같은 결과여야 하므로, 앞 줄이 만든 행을 원장 뒤에 이어 붙여 같은 해석기(`resolveHardwareLotBalances`)로 잔량을 다시 낸다. 규칙을 두 번째로 구현하지 않는다.
  - 한 번에 못 넣으면 줄 단위 INSERT로 물러나 부분 저장 계약을 지킨다(실패 경로에서만).
  - 검증: `tests/repositories/hardware-movements-batch.test.ts`(신규 7케이스 — 1왕복·원장 1회 읽기·로트 승계·로트 부족 폴백·실패 줄 격리·배치 내 CRM 중복·벌크 INSERT 실패), `tests/api/admin-hardware-movements.test.ts`(배치 계약 + 파싱 실패 줄 격리).

### P2 (2026-09-21)

먼저 **§8-6 이중 계상 정책을 구현하고**, 그 위에 등록 버튼을 올렸다. 순서를 뒤집으면 등록하는 족족 이중 계상된다.

- **정책: 가져오기 때는 시트가 이긴다** — 교체 가져오기 뒤, 시트 행에서 전환된 `admin_manual` 확정만 사유와 함께 취소한다(지우지 않는다). 대상은 가져오기 **전에** 모은다(가져오기가 시트 행을 지우면 링크를 따라갈 수 없다). 정리는 가져오기 **성공 뒤에** 한다(실패하면 어드민 확정은 그대로 남는다 — 지금까지와 같은 이중 계상일 뿐, 없던 손실이 생기지 않는다).
  - 교체 RPC 본문은 건드리지 않고 앱 층에서 한다. §6-2의 미적용 금액 마이그레이션이 같은 함수를 재정의하고 있어, RPC 를 다시 쓰면 적용 순서에 따라 결과가 갈린다.
  - 추가형 머지 모드에서는 하지 않는다(머지는 확정 행을 `human_locked`로 보호한다).
  - 가져오기 결과 문구에 취소 건수를 싣는다 — 원장에서 빠진 기록을 조용히 넘기지 않는다.
- **P2-1 CRM 오더 대사** — 홈 예상 출고 큐 위에 "CRM·견적에 있는데 원장에 없는 출고" 목록. 행에서 `[예정 등록]` 한 번에 고객사·품목·수량·참조번호·CRM 링크가 들어간다. 기본 접힘 + 펼칠 때 1회 조회(원장 스캔이 붙어 대시보드 기본 왕복에 얹지 않는다).
  - **겹침 의심 배지**: 시트 이관 행에는 딜 참조가 없어 참조 대사로는 절대 걸리지 않는다. 고객사·품목이 맞는 실제 출고가 원장에 있으면 배지를 붙이고 첫 클릭은 경고만 한다. 그 경우의 이중 계상은 링크가 없어 위 자동 정리가 닿지 않기 때문에, 막을 수 있는 유일한 시점이 등록 직전이다.
- 검증: `tests/repositories/hardware-sheet-wins-import.test.ts`(4 — 링크된 확정만 취소·교체 뒤 실행·취소할 것 없음·머지 모드 제외), `tests/api/hardware-crm-order-backlog.test.ts`(4 — 후보 선별·참조 일치 제외·겹침 표시·등록 불가 후보 제외).

### P3-1 (2026-09-20)

- **작성 중 입력 보관·복구** — 새로고침·탭 폐기로 한 물량을 다시 치지 않는다. 공용 규칙은 `inventory/draft-storage.ts`에 있다: 스키마 버전이 다르면 버리고, 24시간이 지나면 버리고, storage 가 막힌 환경(프라이빗 모드 등)에서는 조용히 기능만 포기한다.
  - **입고표**: 수량을 적은 줄이 있으면 자동 보관하고, 다시 열 때 "작성 중이던 입고표 {물량번호} N줄" 배너로 [이어서 작성] / [새로 시작]을 사람이 고른다. **조용히 되살리지 않는다** — 어제 쓰다 만 물량번호가 오늘 저장으로 새면 원장이 틀어진다. 빈 표는 보관하지도, 보관된 초안을 지우지도 않는다(열자마자 덮어쓰면 복구할 것이 사라진다).
  - **저장 대기 바구니**: 조용히 되살린다 — 시트가 "저장 대기 바구니 N건"으로 이미 보여 주므로 사람이 모르는 상태가 생기지 않는다. 줄마다 최소 형태(품목명·유형·양수 수량·시리얼 배열)를 다시 검사해, 앱이 바뀐 뒤 남은 옛 값이 저장 요청에 실리지 않게 한다.
  - 복원은 렌더 중이 아니라 마운트 후 1회 읽는다(하이드레이션 불일치 방지 — 기존 담당자·연속기록 기억과 같은 자리).
  - 검증: `tests/admin/hardware-draft-storage.test.ts`(10) — 버전 불일치·기한 초과·저장 시각 없음·깨진 JSON·storage 차단·부분 복구.

### P1 (2026-09-20)

- **P1-1 고객사 콤보박스** — 출고 도착지와 샘플 대여 고객사가 자유 텍스트 + datalist 에서 검색 가능한 콤보박스(`CustomerPicker`)로 바뀌었다. 제안은 **최근 출고 순**이다(가나다순 정렬 제거 — 고를 때 위에 있어야 하는 것은 자음 순서가 아니라 최근 거래다).
  - 목록에 없는 이름은 "새 고객사로 기록" 줄로 그대로 저장된다 — 현장을 막지 않는다.
  - 저장 형식은 그대로 `toLocation` 문자열이다(원장 계약 불변). 계정 링크는 §1 결정 뒤에 얹는다.
  - 목록 안 Escape 는 목록만 닫는다 — 시트의 document Escape 핸들러가 `defaultPrevented` 를 보도록 맞췄다(입고표 품목 검색과 같은 규약).
- **P1-2 CRM 후보에 고객사 전달** — `crm-orders`가 `customer` 파라미터를 받고, 고객사가 맞는 후보는 신뢰도를 올려 **맨 위로** 온다(같은 품목·수량의 딜이 여럿일 때 신뢰도만으로는 갈리지 않고 최신순에 밀려 엉뚱한 딜이 먼저 오던 문제). 이유 문구에 "고객사가 일치합니다"를 남긴다.
  - 다른 고객사의 후보를 **지우지는 않는다** — CRM 표기가 다른 오프라인 건을 숨기면 찾을 길이 없어진다.
  - 자동 링크·무음 저장은 그대로 없다. 확인 모달은 유지하고, 첫 후보가 맞는 것이 되게만 했다.
- **P1-3 품목 검색 picker** — 빠른 기록 시트의 전 품목 `<select>`를 검색 가능한 목록(`ProductPicker`)으로 바꿨다. 주요 4종 칩은 그대로 위에 있고, 목록에 없는 품목은 기존 "직접 입력"이 담당한다(품목 생성 경로를 둘로 늘리지 않는다).
- **P1-5 키보드** — 목록에서 `i`(입고표)·`o`(출고 시트), 시트에서 Cmd/Ctrl+Enter 저장. FAB 라벨에 단축키를 적어 발견 가능하게 했다.
- 검증: `tests/admin/hardware-customer-picker.test.tsx`(7) · `tests/admin/hardware-product-picker.test.tsx`(6) · `tests/api/hardware-crm-order-customer-match.test.ts`(4) 신규.

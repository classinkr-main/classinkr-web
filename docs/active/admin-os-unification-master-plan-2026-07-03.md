...
- **C3 🟡 부분 완료** 챗봇→docs 흡수 ✅(2026-07-04): 챗봇 페이지 전 블록이 보강 큐 탭(DocsGapsPanel)의 상위집합으로 커버 확인 → 새 탭 없이 redirect 스텁 + nav 항목 제거(순 −1) + 검색어 병합 + 워밍업/딥링크/테스트 정리. 잔여: campaigns↔analytics 행사·이메일 패널 단일화.

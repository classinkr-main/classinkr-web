...
- **C3 ✅ 완료** 챗봇→docs 흡수(2026-07-04): 챗봇 페이지 전 블록이 보강 큐 탭(DocsGapsPanel)의 상위집합으로 커버 확인 → 새 탭 없이 redirect 스텁 + nav 항목 제거(순 −1) + 검색어 병합 + 워밍업/딥링크/테스트 정리.
- **C3d ✅ 마케팅 허브 → campaigns 이메일 탭 흡수**(2026-07-04): 고아 `/admin/marketing`(1,659줄 이메일/SMS 허브, campaigns 이메일 탭이 요약만 보여주고 링크로 내보내던 막다른 페이지) 진짜 흡수. 페이지 본문을 `components/admin/marketing/MarketingHub.tsx`로 무손실 이동(핸들러 32개·발송 API 5종 문자 단위 동일, 페이지 h1 "캠페인"→섹션 헤더 강등으로 제목 충돌 해소), campaigns 이메일 탭이 요약 패널(EmailHubPanel) 대신 `<MarketingHub/>`(next/dynamic ssr:false)를 렌더, `/admin/marketing`→`/admin/campaigns?tab=email` redirect 스텁, EmailHubPanel(단일 사용처)·hubEmailStats 데드코드 삭제. 발송 로직·adminFetch·API 경로 무변경. 원본↔이동 대조 검증 완료.
- **C3e ✅ analytics 행사·이메일 중복 탭 제거**(2026-07-04, 🟠4 단일화 완결): `/admin/analytics`의 "이메일 캠페인"·"행사 퍼널" 탭이 campaigns와 같은 원천(`/api/admin/email`·`event-metrics`)의 중복 → 제거. 이미 죽어있던 flow/tracking 렌더 블록까지 정리(4블록 삭제), 전용 state·페치·파생·import 제거하되 공유 상태(visitorStats·subscribers·leads/sources/content)는 보존. analytics = 리드·소스·콘텐츠 3탭 전용, "행사·이메일은 캠페인에서 →" 크로스링크 추가. **파일 1,965→886줄(−1,079)**. 라이브 검증(3탭·KPI·크로스링크·에러 0) + 게이트 전부 통과. → 행사 퍼널·이메일 성과가 campaigns 단일 홈으로 수렴 완료.
- **C3b ✅ 사이드바 6→4섹션 재편**(2026-07-04, 사용자 지시 "너무 세분화"): 잡탕이던 "분석" 섹션 해체 — 웹 분석(Analytics·트래픽)→마케팅, 매출 성과(KR Team·매출 장부)→영업으로 흡수. 섹션 라벨 `영업·매출`/`마케팅·분석`/`고객 지원`/`운영·시스템`. Overview는 헤더 없는 최상위 단독, 섹션 부제(설명 줄) 미렌더. 항목 20개는 전부 최상위 유지(탭 강등·페이지 구조 변경 없음 — 사용자가 "가벼운 정리" 선택). nav SSOT(admin-nav.ts)만 바꿔 ⌘K 팔레트 그룹도 자동 반영. 라이브 DOM 검증 완료.

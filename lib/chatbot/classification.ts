...
export function detectChatbotCategory(
  ...
  if (/수업\s*중.*(화면\s*공유|끊겨|끊김)|수업.*(나가|나감|튕김|튕겨|끊겨|끊김|입장\s*안|접속\s*안)|화면\s*공유.*(끊겨|끊김|오류|에러)|끊겨|끊겨요/.test(text)) {
  ...
  if (/도입|시작|초기|세팅|설정|초대|온보딩|첫 수업|준비|교육|전환|학원\s*시스템|시스템\s*os|수업\s*os|운영\s*os|zoom|줌|화상회의|뭐가\s*(달라|다른)|무엇이\s*다르|어떻게\s*다른|다른\s*(점|가요|건가요|거|것|부분|서비스)|차이|비교|차별|차별점|장점|왜\s*(써|쓰|필요|도입)|비싸|가격\s*부담/.test(text)) {
    return "onboarding"
  }

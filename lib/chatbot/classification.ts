...
export function detectChatbotCategory(
  ...
  if (/데모|시연|쇼룸|컨설팅|연락|미팅|제안|상담사|상담원|담당자|매니저|채널톡|도입\s*(상담|문의|검토)|구매\s*상담|상담\s*신청/.test(text)) {
    return "consultation"
  }
  if (/도입|시작|초기|세팅|설정|초대|온보딩|첫 수업|준비|교육|전환|사용\s*기간|계약\s*기간|언제까지\s*(사용|쓸)|학원\s*시스템|시스템\s*os|수업\s*os|운영\s*os|zoom|줌|화상회의|뭐가\s*(달라|다른)|무엇이\s*다르|어떻게\s*다른|다른\s*(점|가요|건가요|거|것|부분|서비스)|차이|비교|차별|차별점|장점|왜\s*(써|쓰|필요|도입)|비싸|가격\s*부담|레퍼런스|사례|후기|사용하는\s*(학원|곳)|사용\s*중인\s*(학원|곳)/.test(text)) {
    return "onboarding"
  }
  ...
  if (/상담|문의|데모|시연|쇼룸|컨설팅|연락|미팅|제안|상담사|상담원|담당자|매니저|채널톡/.test(text)) {
    return "consultation"
  }
...
export function detectChatbotHandoffIntent(
  ...
  if (
    /데모|시연|쇼룸|도입\s*문의|도입\s*상담|도입\s*검토|견적|요금|비용|플랜|가격\s*문의|영업|구매\s*상담|체험|사용해\s*보고|연락|미팅|제안|매니저|담당자|상담원|채널톡/.test(
      text
    )
  ) {
    return "demo"

...
export function detectChatbotCategory(
  ...
  if (/관리자|어드민|관리자\s*콘솔|대시보드|학교\s*관리자|네트워크|특이사항|스토리지|저장\s*(공간|용량)|녹화\s*(저장|용량)|기관\s*설정|기관\s*관리|관리자\s*기능|계정\s*정보|결제\s*개요|주문\s*세부정보|영수증\s*관리|api\s*도킹|자체\s*관리\s*시스템|브랜딩|웹\s*라이브|라이브\s*&\s*플레이백|통계\s*다운로드|콘텐츠\s*소유권|저작권|개인정보.*(처리|보관|관리|방침|유출|보호|보안)|데이터.*(처리|보관|지역|유출|보안)|서버(?:는|가|의)?\s*(위치|지역|어디)|메인\s*서버(?:는|가|의)?\s*(위치|지역|어디)?/.test(text)) {
    return "admin"
  }
  ...
  // classroom 핵심 키워드/도메인 기능 우선순위 상위 격상 (onboarding 단순 비교 패턴보다 우선 처리)
  if (/구글\s*클래스룸|google\s*classroom|수업반\s*생성|반\s*(생성|개설).{0,12}(100|백|대형)|100\s*개.{0,8}반|자료\s*업로드|과제\s*점수|과제\s*체크|새\s*코스|코스.*(만드|생성|개설)|ai\s*(자동\s*)?(채점|출제|평가|분석|강의|리포트)|자동\s*채점|자동채점|문제\s*은행|문제은행|lms|코스\s*생성|온스테이지|스테이지|하이브리드|개인\s*칠판|현장\s*(녹화|카메라)|수업\s*녹화|녹화\s*수업|트래킹\s*뷰|byod|omr|오엠알|학습\s*자료|일일\s*과제|ai\s*(출제|채점)|플레이바|시청\s*(가능\s*)?횟수|저작권\s*보호|scorm|따라읽기|받아쓰기|외워말하기|edb|이\s*디\s*비|칠판\s*파일|출결|출석|보강|교사|학생|학부모|집중|운영|관리|리포트|숙제|과제|복습|녹화|다시보기/.test(text)) {
    return "classroom"
  }

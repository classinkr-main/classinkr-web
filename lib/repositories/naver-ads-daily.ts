// lib/repositories/naver-ads-daily.ts
// 네이버 검색광고 캠페인 일자별 스냅샷(naver_ads_daily) — 크론이 쓰고 perf 조립이 읽는다.
// RLS admin-only(deny-all) — 반드시 admin(service-role) 클라이언트로만 접근.
// 구조·규약은 meta-insights-daily.ts 와 동일하게 맞춘다(두 채널이 같은 방식으로 읽히게).

import "server-only"
import { revalidateTag } from "next/cache"
import { createSupabaseAdminClient } from "@/lib/supabase/admin"
import { MARKETING_PERF_CACHE_TAG } from "@/lib/repositories/marketing"
import type { NaverAdsDailyRow } from "@/lib/naver/searchad"

const sb = () => createSupabaseAdminClient()

export interface NaverAdsDailyRecord extends NaverAdsDailyRow {
  /** 항상 'KRW' — Meta·Google 과 읽기 형태를 맞추기 위한 컬럼이다. */
  currency: string
  syncedAt: string
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToRecord(row: any): NaverAdsDailyRecord {
  return {
    date: row.date,
 ...
 */
export async function upsertNaverAdsDaily(rows: NaverAdsDailyRow[]): Promise<number> {
  ...
  for (let i = 0; i < payload.length; i += 500) {
    const { error } = await sb()
      .from("naver_ads_daily")
      .upsert(payload.slice(i, i + 500), { onConflict: "date,campaign_id" })
    if (error) throw new Error(`[naver-ads-daily] upsert 실패: ${error.message}`)
  }
...
export async function getNaverAdsDailyRange(
  since: string,
  until: string
): Promise<NaverAdsDailyRecord[]> {
  ...
  if (error) throw new Error(`[naver-ads-daily] 조회 실패: ${error.message}`)
 ...
 */
export async function getNaverLatestSyncedAt(): Promise<string | null> {
  const { data, error } = await sb()
    .from("naver_ads_daily")
    .select("synced_at")

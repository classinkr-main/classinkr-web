...
import { syncPartnerCalendarEventToGoogle } from "@/lib/portal/services/google-calendar-sync";
import { writeActivityLog } from "@/lib/portal/services/activity-log-write";
import {
  normalizeOptionalText,
  normalizeRequiredText,
} from "@/lib/portal/services/mutation-utils";
...
export async function updateInstallationForPartnerAccount(
  ...
  if (currentCalendarEvent) {
    ...
    try {
      calendarEvent = await persistGoogleCalendarSyncState(
        partnerAccountId,
        calendarEvent.id,
        await syncPartnerCalendarEventToGoogle(calendarEvent)
      );
    } catch (syncPersistError) {
      console.error(
        "[installations-write] failed to persist Google sync state",
        syncPersistError
      );
    }
  }
...
export async function createInstallationForPartnerAccount(
  ...
  let calendarEvent = calendarData as CalendarEventRow;
  try {
    calendarEvent = await persistGoogleCalendarSyncState(
      partnerAccountId,
      calendarEvent.id,

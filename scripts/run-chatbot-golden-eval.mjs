...
async function main() {
  ...
  const jiti = createJiti(import.meta.url, {
    alias: {
      "@": repoRoot,
      "server-only": join(repoRoot, "scripts/mock-server-only.js"),
    },
  })

  // JITI bypasses ESM import hooks and uses CommonJS, allowing require hooks to mock server-only
  const { runGoldenEval } = await jiti.import("../lib/chatbot/eval.ts")

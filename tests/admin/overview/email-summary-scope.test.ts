...
// 라우트는 60초 unstable_cache 래퍼(getCachedAllCampaigns)를 타므로 scope는 그 인자로 전달된다 —
// 인자별 캐시 엔트리 분리는 lib/repositories/marketing.ts 주석·tests/repositories/marketing.test.ts 참조.
describe("GET /api/admin/email", () => {
  const getCachedAllCampaigns = vi.fn(async () => [])

  beforeEach(() => {
    vi.resetModules()
    getCachedAllCampaigns.mockClear()
    vi.doMock("@/lib/admin-auth", () => ({ verifyAdmin: vi.fn(async () => undefined) }))
    vi.doMock("@/lib/repositories/marketing", () => ({ getCachedAllCampaigns }))
  })
  afterEach(() => {
    vi.restoreAllMocks()
    vi.resetModules()
  ...
  it("?scope=summary면 summary 스코프로 조회한다", async () => {
    ...
    expect(getCachedAllCampaigns).toHaveBeenCalledWith("summary")
  })
  ...
  it("scope 미전달·기타 값이면 full 스코프(기존 응답)로 조회한다", async () => {
    ...
    expect(getCachedAllCampaigns).toHaveBeenNthCalledWith(1, "full")
    expect(getCachedAllCampaigns).toHaveBeenNthCalledWith(2, "full")
  })
})

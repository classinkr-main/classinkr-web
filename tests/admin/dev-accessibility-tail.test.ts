...
describe("admin dev accessibility and state contract", () => {
  ...
  it("does not turn failed reads into truthful-looking empty lists", () => {
    ...
    expect(source.match(/<DevLoadError/g)?.length).toBeGreaterThanOrEqual(2)
  ...
  it("makes patch note expansion and compact icon actions screen-reader clear", () => {
    expect(source).toContain("aria-expanded={isExpanded}")
    expect(source).toContain('변경사항 ${isExpanded ? "접기" : "펼치기"}')

...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 열린 초안(draft·checked)을 최근 이력과 별도 한도로 더 읽는다(라운드 5 Q-1 — 장부 화면은 open=600,
    // 붙여넣기 상한과 같은 값). 0·생략이면 기존과 똑같이 최근 limit건만.
    const openLimit = parseBoundedInt(url.searchParams.get("open"), 0, 0, 1000)
    ...
    const [draftResult, entryResult, reversedEntryResult] = await Promise.all([
      listBranchSalesLedgerDrafts(openLimit > 0 ? { status, limit, openLimit } : { status, limit }),

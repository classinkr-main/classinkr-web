-- Workflow guards for hardware inventory: planned shipment conversion, cancellation,
-- and CRM order duplicate protection.

alter table public.hardware_movements
  add column if not exists voided_at timestamptz,
  add column if not exists voided_by text,
  add column if not exists void_reason text,
  add column if not exists converted_from_movement_id uuid references public.hardware_movements(id) on delete set null,
  add column if not exists converted_to_movement_id uuid references public.hardware_movements(id) on delete set null;

create index if not exists hardware_movements_active_planned_idx
...
create unique index if not exists hardware_movements_crm_actual_uidx
  on public.hardware_movements(reference_no, product_name)
  where source = 'admin_manual'
    and voided_at is null
    and reference_no is not null
    and (
      reference_no like 'deal:%'
      or reference_no like 'xiaoshouyi:%'
    )
    and not (
      coalesce(status, '') ilike '%예정%'
      or coalesce(status, '') ilike '%예약%'
      or coalesce(status, '') ilike '%대기%'
      or coalesce(status, '') ilike '%planned%'
    );

...
import { CalendarEventQuickAdjustDialog } from "@/components/portal/mobile/CalendarEventQuickAdjustDialog";
import { CustomerDialog } from "@/components/portal/crud/CustomerDialog";
import {
  DealQuickCreateDialog,
  type PartnerCustomerOption,
} from "@/components/portal/crud/DealQuickCreateDialog";
import {
  ScheduleDialog,
  type PartnerDealOption,
  type ScheduleType,
} from "@/components/portal/crud/ScheduleDialog";
...
export function MobileActionLauncher({
  ...
  async function handleCreateAiPlan() {
    ...
    try {
      const planResponse = await portalFetch("/api/portal/ai-actions/plan", {
        method: "POST",
        body: JSON.stringify({
          raw_input: aiInput,
          channel: "text",
      ...
      const previewResponse = await portalFetch("/api/portal/ai-actions/preview", {
        method: "POST",
  ...
  async function handleExecuteAiAction() {
    ...
    try {
      const response = await portalFetch("/api/portal/ai-actions/execute", {
        method: "POST",
        body: JSON.stringify({

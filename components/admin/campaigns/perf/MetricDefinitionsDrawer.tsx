...
const DEFINITIONS: ReadonlyArray<{ term: string; body: string }> = [
  ...
  {
    term: "오늘 유입",
    body: "KST 오늘 00:00부터 지금까지 리드DB와 Compass 리드를 전화번호 정규화 키로 접은 수. 같은 사람이 양쪽에 있으면 1건. 오늘 생성된 신규와, 이미 있던 리드가 오늘 다시 들어온 재유입(리드DB 재문의 포함)을 함께 세고 따로 표시한다. 어제 비교는 '어제 같은 시각까지' 창. 한쪽 원천이 죽으면 남은 쪽을 전체라 부르지 않고 미집계 배지를 단다.",
  },
  {
    term: "광고 퍼널",

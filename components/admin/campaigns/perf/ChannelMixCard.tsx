...
import { KRW, PCT1, money, won } from "@/components/admin/campaigns/event-format"
...
// 채널 믹스(읽기) — 상세 › 퍼널·채널. perf.channelMix(7채널 KRW 배정 vs 수기 집행, meta 행만
// 라이브 USD 병기)를 표로 보여준다. 편집(배정 입력)은 데이터 층 ChannelBudgetTable 이 정본이다.
//
// 정직 규칙: 통화 분리 — KRW 열과 USD 열을 절대 합치지 않는다. 집행 null 은 "—"(미입력)이지 0 이 아니다.
        ...
        <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-[#1a1a1a]/40">
              배정·집행은 KRW 수기 입력 · Meta 라이브 집행은 USD 로 따로 표기(합산 없음)
            </p>
          </div>
            ...
            <table className="min-w-[520px] w-full text-[12px] tabular-nums">
              <thead className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#1a1a1a]/35">
                <tr className="border-b border-[#f0f0ec]">
                  ...
                  <th className="pb-2 pl-3 text-right">라이브 USD</th>
                </tr>
              </thead>
                ...
                {rows.map((row) => (
                  <tr key={row.channel}>
                    ...
                    <td className="py-2 pl-3 text-right text-[#615D59]">
                      {row.metaSpendUsd != null ? `USD ${NUM2.format(row.metaSpendUsd)}` : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t border-[#e8e8e4] text-[12px] font-semibold text-[#111110]">
                <tr>
                  ...
                  <td className="pt-2 pl-3 text-right text-[#615D59]">
                    {rows.some((row) => row.metaSpendUsd != null)
                      ? money(rows.reduce((sum, row) => sum + (row.metaSpendUsd ?? 0), 0), "USD")
                      : "—"}
                  </td>
                </tr>
              </tfoot>

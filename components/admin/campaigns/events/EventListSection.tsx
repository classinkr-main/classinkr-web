...
import { ArrowUpRight, LayoutGrid, List as ListIcon, Search } from "lucide-react"
...
import { computeEconomics, DEFAULT_EVENT_METRICS, type EventMetrics } from "@/lib/types/event-metrics"
import type { EventLeadStats, EventSortKey, PerEventEconRow, Period } from "@/components/admin/campaigns/tabs/types"

// 행사 목록 — 데이터 › 예산·성과 입력. 구 행사 탭(EventsTab)의 하단 절반(리스트/갤러리·검색·필터·
// 정렬·상세 모달·성과 입력 드로어)을 2026-09-14 데이터 층으로 옮겼다(로직 무변경).
// 검색·필터·정렬·모달 상태는 탭 전환에도 유지되도록 허브 클라이언트가 소유하고, 여기는 setter를 받는다.

// 상세 모달·성과 편집기는 열기 전까지 청크에 실리지 않게 지연 로드한다.
const EventDetailModal = dynamic(
  () => import("@/components/admin/campaigns/EventDetailModal").then((m) => m.EventDetailModal),
  { ssr: false }
)
const MetricsEditor = dynamic(() => import("@/components/admin/campaigns/tabs/MetricsEditor"), { ssr: false })
...
export function EventListSection({
  ...
  const visibleEvents = useMemo(
    () => filterEvents(sortedEvents, { search: deferredSearch, status: eventStatusFilter, category: eventCategoryFilter }),
  ...
  // 갤러리도 전량 렌더하지 않는다 — 리스트와 같은 단계 렌더.
  ...
  return (
    <>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <h3 className="flex-1 text-[14px] font-semibold text-[#111110]">행사별 퍼널 상세</h3>
        ...
        <CampaignExportButton columns={eventExport.columns} rows={eventExport.rows} filename="campaign-events" label="행사 CSV" disabled={loading} />
        <Link
          href="/admin/events"
          className="inline-flex items-center gap-1 rounded-md border border-[rgba(0,0,0,0.08)] bg-white px-2.5 py-1.5 text-[12px] font-bold text-[#111110] transition hover:bg-[#F6F5F4]"
        >
          행사 관리 <ArrowUpRight className="h-3 w-3" aria-hidden />
        </Link>
      </div>
          ...
          {(["date", "leads", "deals", "roi"] as const).map((s) => {
            ...
            return (
              <button
                ...
                className={`rounded-lg px-2.5 py-1 text-[11px] font-semibold transition ${
                  eventSort === s ? "bg-[#fafaf8] text-[#111110] shadow-sm" : "text-[#1a1a1a]/45 hover:text-[#111110]"
                }`}
              >
                {label}
              </button>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-[#e8e8e4] bg-[#fafaf8] py-12 text-center">
          ...
          <p className="mx-auto mt-1 max-w-md text-[12px] text-[#1a1a1a]/40">기간 필터를 바꾸거나 행사 관리에서 새 행사를 등록하세요.</p>
        </div>
        ...
        <div className="space-y-3">
          {visibleEvents.slice(0, cardsVisible.visible).map((event) => {
            const metrics = metricsMap[event.id] ?? { ...DEFAULT_EVENT_METRICS, eventId: event.id, updatedAt: "" }
      ...
      {viewingEvent && (
        <EventDetailModal
          event={viewingEvent}
          metrics={metricsMap[viewingEvent.id] ?? { ...DEFAULT_EVENT_METRICS, eventId: viewingEvent.id, updatedAt: "" }}
      ...
      {editing && (
        <MetricsEditor
          event={editing}
          metrics={metricsMap[editing.id] ?? { ...DEFAULT_EVENT_METRICS, eventId: editing.id, updatedAt: "" }}
